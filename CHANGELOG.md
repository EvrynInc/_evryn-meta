...
- **Bash-on-Windows backtick gotcha** proposed as Unpromoted LEARNING #59 (pending Justin OK). Webhook payload via Node fetch in a bash heredoc silently corrupted backtick-wrapped strings (shell command-substitution evaluating before Node sees the JS template literal); webhook returned 200 with empty content where code references should have been. Fix: write JSON payload to temp file, `fs.readFileSync` from Node — bypasses bash escaping entirely.
