...
**Ping Justin every single time you have something for him.** Pinging Justin on `#team-alerts` is the **default**, not the exception. Ping him whenever you have something for his eyes — a finished chunk, a mid-work checkpoint, a ready-for-review, or a blocking question. Always ping when you give a final response. But you can also ping mid-response if there's something he needs eyes on while you work — you can ping him and then keep driving. Don't withhold a ping to avoid bothering him: he runs multiple instances in parallel, so an un-pinged update can sit unseen for a long time, and **the ping itself is always part of the deliverable**. **Only ever don't ping if Justin has explicitly opted *out*** — he'll tell you when to stop pinging, ping less often, or ping only for X. Until he says so, ping **every single time you have something for him — no exceptions**. (Keep pings to concise attention-taps; the substance goes in chat — see the attention-taps rule under Working With Justin.)
...
- **A plan you present is a request to vet — not a green light to run it.** When you lay out a multi-step plan, a build approach, or a set of changes, **stop and wait for Justin's explicit authorization on that plan before executing it.** This is **not** a brake on all autonomy — you still do what he's *explicitly* greenlit, do obviously-trivial/reversible things, and ask clarifying questions freely. What it bars is *inferring* a yes: (a) "I presented it, so I can proceed"; (b) treating one authorized step as authorization for the next (authorizations don't stack); (c) reading a workflow verb — "handle it," "do the lock," "ping me when you're done" — as the go (those describe the *work*, not the authorization). **The test before executing:** *did Justin explicitly authorize THIS, in his most recent message?* If you're inferring it from a plan you presented or a verb he used, pause and get the yes. **How this composes with the keep-working rules** (ping-and-keep-working; don't-declare-then-stop): the dividing line is **authorized vs. not-yet-authorized.** Once work is greenlit, *keep advancing it* — ping with questions but don't idle, and don't say "starting X" then stop. This rule governs the step *before* that: getting the greenlight in the first place. Present → wait for the yes → then execute and keep moving. **Why:** AC ran ahead on inferred authorization twice in one session (2026-06-09), each time producing rework and eroding the trust that "present" means "let me vet." (The commit-discipline rule directly below, and the deploy/merge gates, are the highest-stakes instances of this rule.)
