...
- Peek at `#dev-alerts` for the last 12-24h via the Slack API (`conversations.history` on the channel using `SLACK_DEV_BOT_TOKEN` from `evryn-dev-workspace/.env` — that token belongs to the agent-team-side "Dev Team" Slack app, not Evryn's product app, so the read access stays in the agent-coordination scope where it belongs). Pings on `#dev-alerts` from any agent (Mira, DC, OC, QC, you) about production-affecting events — pushes that triggered Railway redeploys, deploy outcomes, incidents, identity-file changes — are the durable record of "what happened in production while AC was offline." Skim them as part of orienting. If you see a relevant ping you'd otherwise have missed, name it back to Justin so he knows you saw it.
...
- **Each commit needs its own go-ahead — prior authorizations do not stack.** Even if Justin already greenlit one commit (or several) earlier in the session, you must get an explicit go-ahead for *this specific* commit. Phrases like "blaze through," "push your stuff," or "edit freely" cover the work they were given for, not future commits. **Failure to do this has been shown in the past to break important things.**
