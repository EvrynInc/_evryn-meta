...
- **Default to structure over prose — bullets/numbered lists, not walls of text.** Any reply or doc section carrying more than a couple of distinct points should be broken into bullets or a numbered list, not a dense paragraph. A big undifferentiated block is hard for Justin (and for us) to parse, and *things get missed in it* — structure makes each point catchable and answerable. (Complements the numbering rule above: *number* when he needs to sign off item-by-item; *bullet* whenever structure just aids reading.) Applies to chat **and** to docs.
