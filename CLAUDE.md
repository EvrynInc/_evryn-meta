...
- **The active product's runtime — BOTH halves: `evryn-backend/src/` AND `evryn-backend/identity/*.md`.** ⚠️ **The identity files ARE runtime — the "judgment runtime" — and they are the half everyone forgets.** An identity instruction programs Evryn's behavior as surely as a line of code: what she does, what tools she calls, what she must never do. A `src/`-only load is a **half load** of the system, and you will not be able to tell — you'll reason confidently about a system whose behavior is half-written in files you skipped. **They are never optional, never "just voice docs," and never inferable from their filenames.** Note the trap in enumerating: `find src -name "*.ts"` **cannot** return an identity file — so any live-enumeration recipe silently omits them unless you enumerate `identity/` too. Currently 11 files (`core.md` · `activities/*` · `situations/*` · `internal-reference/*` · `public-knowledge/*`) — but **enumerate live rather than trusting that count.** *(A real defect class: an identity file instructing Evryn to call a tool the runtime no longer has — invisible unless you've read both halves. Live example, 2026-07-16: Step 57 deleted `record_pass` from the code while `triage.md` still told her to call it on every pass.)* ARCH says the *intended* shape; the runtime is the *actual* shape, and the two regularly diverge in load-bearing ways. We've tried to load it only on demand, and 99 out of 100 times we end up breaking things because we're making decisions blind. The divergence is often exactly what bites. **And ask *which* runtime — every time.** `src/` on the default branch is the *shipped* shape; in-flight work lives on feature branches / worktrees and can differ in load-bearing ways (whole modules added, rewritten, or deleted). Decide up front whether the task needs the default branch, a specific branch/worktree, or both; if it's unclear, *resolve it rather than assume*, then **tell Justin explicitly which runtime state you loaded.** Reading only `main` while the live work is a held bundle on a worktree means orchestrating your work blind. See the "I have a specific assignment" note in the context discipline about what to do when Justin is trying to protect your context. If he hasn't stipulated runtime other than `main`, then read that - but make sure it's clear that that's what you read - he may want to have you read *other* runtime *in addition to `main`, so that you understand the *contrast*, which orients you in your place in the build.  
