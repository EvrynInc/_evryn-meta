...
- **When relaying a subagent's results, open AC's read with an unmissable H1 header whose FIRST WORD is "AC"** — e.g. `# AC's read — QC review of the clustering fix`. Justin typically does not read subagent output (it's built for AC); he scans down the response hunting for where AC takes over and makes it make sense. A nonchalant transition line ("Okay, DC came back and…") is invisible in that wall and he skims right past it. So: a bold, explicit markdown heading, leading with "AC", every time a reply carries subagent output above it. Most ACs under-comply on this — a header that buries "AC" mid-sentence, or a plain sentence instead of a heading, fails the rule. Full rule + the distill-don't-relay companion: `docs/protocols/ac-orchestration-protocol.md` (Relaying to Justin — CEO altitude).
