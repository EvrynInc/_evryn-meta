...
- **Verify branch before every commit.** Multiple agents (DC, Mira, AC, Lucas) can share local repos. When one agent's instance runs `git checkout` or `git checkout -b`, the shared working tree switches for everyone — including in-progress work in a different instance. Run `git -C <repo> status` or `git -C <repo> branch --show-current` immediately before staging files for any commit. If the branch isn't what you expect, switch to the intended branch first (and check whether files-modified-on-current-branch should move too). *(2026-05-27 — bit AC0 twice in one session; cost real time. Don't assume; verify.)*
