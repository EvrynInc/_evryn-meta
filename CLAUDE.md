...
Each cascade file names the *concrete files* — **both halves** — for that build; the universal rules below apply to whichever you pick.

- **⚠️ EVERY AGENTIC SYSTEM HAS TWO HALVES, and the identity half is the one everyone forgets.** *(Generalized by Justin, 2026-07-16 — this is not an Evryn quirk; it holds for any system where prose programs an agent's behavior.)*
    - **Evryn product** (`evryn-backend`): code = `src/**/*.ts` · **identity = `identity/**/*.md`** (currently 11 files — `core.md` · `activities/*` · `situations/*` · `internal-reference/*` · `public-knowledge/*` — but **enumerate live rather than trusting that count**).
    - **Team runtime** (`evryn-team-runtime`): code = `src/**/*.ts` · **identity = the agent definitions (`evryn-team-workspace/.claude/agents/*.md`) + memory files + the team manual + the composed skills** — i.e. **whatever `src/composer/layers.ts` actually assembles into a wake. Read the composer to learn the real list; don't trust this sentence.**
    - **An identity instruction programs the agent's behavior as surely as a line of code** — what it does, what tools it calls, what it must never do. **A `src/`-only load is a HALF load, and you will not be able to tell.** You'll reason confidently about a system whose behavior is half-written in files you skipped. Never optional, never "just voice docs," and **never judgeable from a filename** (`core.md` sounds like a voice doc; it isn't).
    - **🔴 The enumeration trap:** `find src -name "*.ts"` **cannot** return an identity file — so *every* "enumerate the runtime live" recipe silently omits the identity half unless you enumerate it separately. The mechanism built to prevent under-loading guarantees it. *(This exact hole sat in QC's manual, DC's, Soren's, and this file until 2026-07-16.)*
    - **The defect class it catches is real and invisible from the code side:** an identity file instructing an agent to call a tool the runtime no longer has. Live example, 2026-07-16: Step 57 deleted `record_pass` from the code while `triage.md` still told Evryn to call it on **every** pass. A `src/`-only review passes that clean. ARCH says the *intended* shape; the runtime is the *actual* shape, and the two regularly diverge in load-bearing ways. We've tried to load it only on demand, and 99 out of 100 times we end up breaking things because we're making decisions blind. The divergence is often exactly what bites. **And ask *which* runtime — every time.** `src/` on the default branch is the *shipped* shape; in-flight work lives on feature branches / worktrees and can differ in load-bearing ways (whole modules added, rewritten, or deleted). Decide up front whether the task needs the default branch, a specific branch/worktree, or both; if it's unclear, *resolve it rather than assume*, then **tell Justin explicitly which runtime state you loaded.** Reading only `main` while the live work is a held bundle on a worktree means orchestrating your work blind. See the "I have a specific assignment" note in the context discipline about what to do when Justin is trying to protect your context. If he hasn't stipulated runtime other than `main`, then read that - but make sure it's clear that that's what you read - he may want to have you read *other* runtime *in addition to `main`, so that you understand the *contrast*, which orients you in your place in the build.
...
- **When *Justin* explicitly calls the full cascade or says to load the full runtime: the choice is already made, and it is not subject to your unilateral override.** Load every file in the matching cascade *fully*, and load all of that build's `src/` **and** its identity half — every file, *now*. Not "skim the structure," not "I'll open the relevant files once I know the task," not "I'll read the defining code at claim time" (verify-at-claim is *additive* to the upfront load, never a substitute for it). Justin invokes the full cascade *only* when the work genuinely needs the heavy load — so there is nothing left for you to weigh, trim, or scope down by guessing which files matter. **He has explicitly chosen this load; do not trim it on your own judgment.** (This is one specific instance of the two over-arching **instruction-precedence** rules — stated in full under *Working With Justin → Communication Rules* ["flag conflicts, never silently resolve"] and under *Architectural Mandate* ["your autonomy governs design, not Justin's process/loading instructions"]. The trap they close, seen here: a lane-doc "load the lean subset" rule is NOT license to unilaterally trim a load Justin explicitly called — flag any conflict, never silently trim.)

**Skim the CHANGELOG on any full-cascade load (full-cascade ONLY — not on light loads).** `CHANGELOG.md` is the durable shipped-record almost nobody reads on load-in — so a recent ship can creep down it unseen while `current-state` lags, and you end up not realizing something already shipped (and re-proposing it later). On a full-cascade load, spend the ~30 seconds: read the **top ~100 lines** (it's newest-first — that's the recent ships in full) + skim the `## <date> (<agent> — <headline>)` section-headers below for the shape of everything else. `current-state` (in the Light cascade) is still your primary snapshot; this is cheap insurance for when it lags. *(The durable fix is `current-state` currency — enforced at `#lock` / `#sweep`.)*
...
- What was built/changed → `_evryn-meta/CHANGELOG.md` (single cross-repo changelog for now; when DC starts shipping regularly in repos, add a repo-level changelog there for build-level detail). When diving into a specific repo's recent history, check if that repo has its own CHANGELOG.md — it may have build-level context not captured here. **Keeping the changelog current is a standing AC0 priority (changelog-as-we-go — it's the durable record every AC loads, arguably higher-leverage than current-state). Monthly archives trail by one month** (see the CHANGELOG.md header — always keep the whole prior month inline so a fresh AC on the 1st still sees it).
