...
The Hub explicitly plans multi-channel communication: email now, then the Evryn app, then potentially SMS/MMS, Telegram, WhatsApp, etc. (`docs/hub/roadmap.md` line 84, `evryn-team-workspace/shared/projects/legal/terms-and-privacy/2026.02.19-privacy-and-terms-questionnaire.md` Q4 multi-channel section). The ARCHITECTURE.md principle is clear: conversations with the same user across channels interleave by time — same friendship, different channels.
