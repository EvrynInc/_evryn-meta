...
**Post-connection adjustments:** if the connection was better than expected, user can increase payment. If worse, they can decrease — all the way to zero if that's fair. Evryn may insist on a full refund if a connection went badly. "No worries — I've returned your payment to your Evryn Wallet. These things happen. But it will really help me if you tell me what didn't feel right." This is a full-refund policy: any user can get their payment back within 30 days, no explanation required. The mechanism is self-correcting: honest feedback helps Evryn calibrate future matches; dishonest refund claims (claiming a match was bad when it wasn't) degrade Evryn's understanding of what the user actually wants — which means her next matches get worse, not better. The incentive to be truthful is structural.
...
2. **Post-Match Transactions** — user-to-user payments (via Stripe Connect) for follow-up work, tipping, repeat gigs. Evryn can take a small commission. **⚠️ This stream's commission mechanic is gated on a licence we have deliberately deferred. Do not model it as free and frictionless — and do not model it as a near-term cost either.** Per Nathan's California Talent Agencies Act finding, *taking a percentage of an artist's earnings* is the single clearest factor that makes an intermediary a regulated talent agency; a flat, one-time introduction fee is not. **Near-term scoping (Justin, 2026-07-29): Washington only for at least a year, and Evryn does one thing — she introduces people.** What the two parties then choose to do together is theirs. Evryn takes **no cut of anything downstream, one time only** — and whether that one-time payment is even best characterised as a *fee* or as a *gratuity* is an open question Justin has raised. Under that scoping, licensure is **a known future gate on this stream — revisited when there is revenue and overhead to absorb it — not a cost line in the current model.** **Revenue stream 1 (match payments, the core business) is Justin's read as entirely unaffected, pending Nathan's confirmation** — if that holds, one of three streams is gated, later, in a state we will not operate in for at least a year. She's a relationship steward, not a marketplace. Introductions aren't enough — follow-through matters. By facilitating payments and re-engagements, Evryn becomes not just the introducer, but the connective tissue for real working relationships.
...
**Burn rate components** — ⚠️ **the dollar figures in this list are March 2026 estimates and are NOT current. Do not budget from them.** The three-way *decomposition* (base ops · team agents · Evryn product) is still the right way to think about burn; the numbers underneath it have all moved. **For live burn and cash position, read the current-state snapshot** (`evryn-team-workspace/shared/current-state/`) — it owns those figures, and a number restated here will always drift from it.

- **Base operating costs** (infrastructure, services, subscriptions, advisor): modeled at ~$725/mo in March 2026. Current-state structures this differently (base ops plus the Megan advisory retainer as a separate line) — read it there. Itemized tool-by-tool breakdown in [BizOps spoke](bizops-and-tooling.md).
- **AI agents** (Anthropic API costs for the founding team): modeled at ~$800/mo steady state, up to ~$3,500/mo during build sprints. This predates the ADR-050 autonomous runtime; **superseded for the agent layer by** `evryn-team-workspace/shared/projects/ops/team-runtime/2026.05.30-team-agents-runtime-cost-model.md`, which models the team as SDK agents with spend gates tied to revenue triggers.
- **Evryn product** (Anthropic API costs): modeled at ~$1,450/mo "once live." **This was the most misleading number in this section — it understates the real modeled cost by roughly 1.4–4.5×.** The fitted cost model against live v0.2 runtime data puts servicing one gatekeeper at Mark's stated volume (~6,000 forwards/mo) at ~$2,000/mo with a warm prompt cache and ~$6,500/mo cold, post-hardening — and ~$11,000/mo unhardened. **Read** `evryn-team-workspace/shared/projects/product/research/2026.06.11 evryn-cost-analysis.md` — note it carries a 2026-07-28 correction banner (its per-item figures ran ~2× high after the prefix it was fitted against was halved), so read the banner before using any figure in it.

> **[QUARANTINED — everything from here to the end of "The Seattle multiplier" is a March 2026 model. Superseded, never realized. Do not plan against it.]**
>
> **Scope note — this quarantine covers the prose as well as the tables.** The three tables — **Burn Profile**, **Revenue Model**, and **Cash Runway Projection** — *and* the four interpretive paragraphs that follow them (**"Reading this table," "Aspiration: agents earlier," "Capital timing," "The AI-first advantage"**) are all part of the same March 2026 model. The prose is the easiest part to misread as a live plan, because it is written in the present tense and sits under headings a reader skims for exactly this information.
>
...
|---------------|-------------|-------------------|
| Base ops only (March 2026 — building, agents paused) | ~$800 | ~7.5 months |
...
### Reading of the March 2026 model — SUPERSEDED (kept for its reasoning, not as a plan)

> ⚠️ **The four paragraphs below were written in March 2026 and are not a current plan.** They are retained because the *reasoning* is still instructive — how the burn tiers interact, why agents-earlier was judged pure upside, why capital was sequenced the way it was. But every forward claim in them failed. Two that a skimmer will otherwise take as live: the **"the cash can support it"** conclusion is false — it depended entirely on the revenue curve in the superseded table above, which never materialized; and **"Capital timing"** describes a schedule that did not happen on those dates. **For the live capital and cash position, read the current-state snapshot** (`evryn-team-workspace/shared/current-state/`).
...
> **What actually happened (dated events and one structural correction — deliberately no balances; current-state owns those):** the founder tranche came in **2026-06-10, not May** — $5,000 drawn, the first *realized* piece of the founder capital that current-state had been carrying as *"$15K unofficially cleared / not yet drawn."*
>
> ⚠️ **The two bullets above describe ONE pool of capital, not two. Never add them together.** **Justin is the angel.** The *"$10K angel"* line that appears across our docs **is** the uncalled remainder of Justin's own founder capital — the $15K, less the $5K drawn 2026-06-10 — **not a separate outside investor stacked on top of it.** Reading them as two pools roughly *doubles* the apparent capital available, and it has already produced a materially wrong runway conclusion once (caught 2026-07-29, after a capital-to-Fenwick comparison was stated as "tight, not impossible" when the corrected figure made it *short*). That remainder is still **contingent on v0.3**, which has not started — so the March premise that "by June, v0.3 will have ~1 month of data" never became true. **For live cash and the current state of that single capital line, read the current-state snapshot** (`evryn-team-workspace/shared/current-state/`).
