...
2. **Post-Match Transactions** — user-to-user payments (via Stripe Connect) for follow-up work, tipping, repeat gigs. Evryn can take a small commission. **⚠️ Do not model this stream as cost-free: the commission is a licensed activity.** Per Nathan's California Talent Agencies Act finding, *taking a percentage of an artist's earnings* is the single clearest factor that makes an intermediary a regulated talent agency — flat introduction fees are fine, a cut of the resulting engagement is not. Justin's decision (2026-07) is to **get licensed and price it in** rather than design around it ("if we try to dance around all of these, that's going to distort us too much"), on the reasoning that a license is also a **moat** — a regulatory barrier every competitor faces too. Licensure is understood to be a bond, an application, a fee-schedule filing, and Labor Commissioner approval of forms; **actual cost and timeline are pending from Nathan and are an input to the rebuilt financial model.** She's a relationship steward, not a marketplace. Introductions aren't enough — follow-through matters. By facilitating payments and re-engagements, Evryn becomes not just the introducer, but the connective tissue for real working relationships.
...
> **What actually happened (dated facts, safe to restate here because they are events, not balances):** the founder tranche came in **2026-06-10, not May** — $5,000 drawn, the first *realized* piece of the founder capital that current-state had been carrying as *"$15K unofficially cleared / not yet drawn."* Roughly $10K of founder capital remains uncalled. **The $10K angel has not come in and remains contingent on v0.3**, which has not started — so the March premise that "by June, v0.3 will have ~1 month of data" never became true. **For the live cash position and the current state of both capital lines, read the current-state snapshot** (`evryn-team-workspace/shared/current-state/`); it owns those numbers and this spoke deliberately does not duplicate them.
