...
### Installment Plans (v0.4 target)

Trust-based pricing asks users to pay what a connection is worth. But some users can't pay the full value upfront. Someone gets connected to a job opportunity they'd honestly price at $200 — but $200 right now isn't realistic. They either pay $20 (leaving $180 of stated value uncaptured) or walk away entirely. Both outcomes underserve the user and leave money on the table.

Installment plans let Evryn offer auto-billing payment schedules for connections whose stated value exceeds what the user can pay at once. Not a subscription, not a debt instrument — just a payment schedule for a price the user already chose. No interest. No penalties. If they stop, they stop.

**Stopping is a conversation, not a default.** Because Evryn maintains an ongoing relationship with every user, installment changes are naturally part of that relationship:

- **Circumstances change.** Someone valued a job connection at $200, but three months in the job falls through. They tell Evryn. She agrees they should stop — the value didn't materialize as expected. She might refund part of what they paid, because that's what a fair broker does.
- **The connection runs its course.** A relationship that felt worth $200 at the start has faded six months later. The user tells Evryn, and she agrees — the stated value reflected an expectation that didn't fully play out. No hard feelings.
- **Someone just stops paying with no explanation.** Evryn notices, the way she notices everything — but doesn't assume the worst. Maybe they forgot, maybe life got hard, maybe they're the kind of person who doesn't follow through. It's one data point among many, consistent with "character becomes currency."

**Why this matters strategically:** Without installments, Evryn can only fully serve users who can pay upfront. That excludes entire market segments of people who genuinely value what Evryn does but don't have the cash flow for a high-stakes connection's full price in one payment. At scale, gratitude drives payment continuation — the connection already worked, the relationship already exists, and ongoing payments feel like honoring that rather than servicing a debt. The churn risk is comparable to any subscription that can be canceled at any time — but installments start from a stated intention anchored to a specific connection's real value, which is a stronger foundation than a generic monthly fee.

**Timing:** v0.3 is too scope-heavy. Target v0.4, or v0.3.1 if bandwidth allows — the market-access argument is too strong to defer indefinitely.

**Seattle cultural resonance:** Trust-based pricing isn't just a business model that works in Seattle — it speaks the local language. The city that birthed REI (a co-op) has a deep "pay what's fair" ethos: strong farmers market culture, progressive economic experimentation ($15 minimum wage pioneer), robust "support local" identity. Simultaneously, Seattle's population is deeply skeptical of being monetized — they house the companies that built surveillance capitalism and understand it viscerally. "You pay for value, we never sell your data" directly addresses a wound this city carries. The anti-corporate, cooperative sensibility extends even more strongly into Portland, where co-ops and worker-owned businesses are part of the cultural fabric. This cultural fit is a growth mechanic: "I paid what I thought was fair and Evryn just accepted it" is a story that gets told at dinner parties. See `evryn-team-workspace/shared/projects/growth/research/2026.03.17 seattle-launch-strategy-v1.md` (Part 6) for full analysis.
...
**Cross-domain matching changes the math further.** The thresholds above assume single-domain matching (enough actors AND directors within film). But Evryn matches across domains — each person has multiple intents (professional, romantic, practical, creative), and any two people might match on an intent neither expected. 300 people across 10 domains might be MORE matchable than 300 in one domain, because the matching surface area is the full product of people × intents, not a single domain silo. Combined with proactive finding (Evryn creates density on demand by recruiting supply-side for whatever users need), the effective density threshold for delivering value is significantly lower than these per-cluster numbers suggest. See [GTM spoke](gtm-and-growth.md) (Concentrate Effort, Open Aperture) for the strategic implications.
...
**The Seattle multiplier:** Every dollar invested in Seattle user acquisition has a geographic multiplier via the spore dynamic (see [GTM spoke](gtm-and-growth.md), The Spore Dynamic). Seattle is a high-throughput migration city — users who leave carry Evryn to their next city, users who arrive tell people back home, and proactive finding means Evryn can serve users anywhere from day one. This means the effective CAC for users in secondary cities approaches zero for spore-driven acquisition. The Seattle investment isn't just buying Seattle users — it's buying seeds for national expansion.
