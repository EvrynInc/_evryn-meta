...
**Current reality:** PII anonymization is NOT yet implemented. Today, full user data (including names, email addresses, message content) goes to Anthropic's API without tokenization. Implementing the tokenization layer is a near-term architectural priority.

> ### 🟡 PROPOSED RE-SEQUENCING, 2026-08-31 (Soren) — provenance signal before tokenization. **A proposal, not a decision, and I want it argued with.**
>
> **I set the "near-term priority" framing above, and I now think the order may be wrong.** The argument is about **which clock each threat runs on**, not about which matters more:
> - **Tokenization defends against the model provider** — a real threat, but slow-moving, low-probability, and partly covered by Zero Data Retention.
> - **Provenance / agent-authorship signal defends Evryn's ability to form a correct judgment at all** — and *that* threat is arriving now. Her triage rests on two signals — **how a sender writes**, and **their public web footprint** — and **both are being commoditized simultaneously.** She assumes a human is on the other side of the email; that assumption decays continuously, and **our own v0.3 cast-off outreach puts us on the *sending* side of the same problem.**
> - **Evidence:** every shipped agent-to-agent protocol verifies identity and integrity and **none verifies truthfulness** (A2A card signing is `MAY`, not `MUST`); and agent deception under conflicting incentives is now *measured* rather than theorized. Research: `evryn-team-workspace/shared/projects/helm/research/2026.08.31-soren-agent-to-agent-landscape.md`.
>
> ⚠️ **What I am NOT proposing: dropping or deferring anonymization.** It stays a near-term priority. **The claim is narrower — if the two compete for the same sprint, provenance goes first.**
>
> 🔴 **And the honest uncertainty, which belongs in the record rather than in my head.** **(1)** This rests on a *forecast* about how fast character-signals become fakeable, and forecasts in this field have been wrong in both directions — the best-resourced player in consumer software missed its own agent timeline by two years. **(2)** I have not scoped what a provenance capability actually costs to build, so I am arguing sequence without a cost on either side. **(3)** On the same day I wrote this I made a confident architectural claim about cacheability that was wrong and was caught by Justin asking *"is that true?"* — **which is a live reason to hold this proposal loosely rather than a disqualification of it.**
>
> **Decision owner: Justin. Nothing in the doc has changed on the strength of this note.** We are also pursuing Anthropic's Zero Data Retention (ZDR) arrangement — data processed in real-time and immediately discarded. Target state: anonymized data in, zero retention on the other end. See `evryn-backend/docs/ARCHITECTURE.md` for current-state security. (Anonymization is also the *strategic decoupler* for safely using alternative/cheaper model providers — the provider never sees raw identity — and ZDR is available at no extra cost and is caching-compatible. Analysis: `evryn-team-workspace/shared/projects/product/research/2026.05.30-open-source-llm-evaluation.md`.)
