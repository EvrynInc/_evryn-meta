...
> ### 🔑 WHAT ANONYMIZATION IS FOR, AND WHEN EACH PIECE IS NEEDED — settled by Soren and Mira, 2026-09-08
>
> **Anonymization defends TWO doors, and the second one is the one that gets forgotten.** The first is the **model provider** — what a third party could see or retain. The second is **Evryn's own composed tool arguments**: in an agentic system the model writes every outbound tool call, so **anything she can see, she can emit** — into a search query, a URL, a draft. **That makes tokenization a bulkhead rather than a filter**, which is why it cannot be replaced by instructing her to be careful.
>
> **The sequence is a question of TRIGGERS, not of order** — each piece has its own moment, and none of them competes with the others for a sprint:
> - **(a) A targeted egress content-check — now.** The nearest exposure is what reaches the open web (`evryn-backend` SPRINT Steps 126–127).
> - **(b) PII tokenization — at gatekeeper go-live or at corpus ingest, whichever comes first.** Both are the moment real personal data arrives at volume.
> - **(c) Provenance / agent-authorship signal — when v0.3 cast-off outreach is scoped.** Evryn's triage assumes a human wrote the email, and that assumption is decaying: every shipped agent-to-agent protocol verifies identity and integrity and **not one verifies truthfulness** (A2A card signing is `MAY`, not `MUST`). **Our own outreach puts us on the sending side of the same problem.** Research: `evryn-team-workspace/shared/projects/helm/research/2026.08.31-soren-agent-to-agent-landscape.md`.
>
> *(This replaces a 2026-08-31 proposal of Soren's to sequence provenance ahead of tokenization. He and Mira withdrew the ordering framing on 2026-09-08 — the exchange is `evryn-team-workspace/shared/projects/helm/standups/2026.09.04-standup/2026.09.08-soren-mira-pii-exchange.md`.)*
>
> We are also pursuing Anthropic's Zero Data Retention (ZDR) arrangement — data processed in real-time and immediately discarded. Target state: anonymized data in, zero retention on the other end. See `evryn-backend/docs/ARCHITECTURE.md` for current-state security. (Anonymization is also the *strategic decoupler* for safely using alternative/cheaper model providers — the provider never sees raw identity — and ZDR is available at no extra cost and is caching-compatible. Analysis: `evryn-team-workspace/shared/projects/product/research/2026.05.30-open-source-llm-evaluation.md`.)
