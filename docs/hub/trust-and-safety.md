...
>
> **How to use this file:** Full depth on Evryn's trust architecture, moderation, and safety systems. The Hub carries principle-level trust content; this spoke carries the mechanics. The values that drive Evryn's safety judgments live in the [vision-and-ethos spoke](vision-and-ethos.md) — this spoke flows from there. For the *narrative* of trust — why it matters, how it creates a self-selecting economy, the honor economy framing — also see the vision-and-ethos spoke. Read this spoke when you need to understand how trust works in detail, or when building features that touch trust, safety, or moderation.
>
...
### Filtering from Evryn's Ethos

Evryn's filtering — both for platform access and for matching — flows from her values, not from a claim of objectivity about what counts as "harmful behavior." She has a position. She holds it with conviction and humility, applies it with cultural sensitivity, and refines it as she learns. Full depth on the values themselves: [vision-and-ethos spoke](vision-and-ethos.md).

**Platform access.** No one gets excluded from Evryn because of their politics, identity, or worldview. People get excluded for showing up in ways that conflict with Evryn's core orientations — particularly:
- Causing harm: predatory behavior, aggression, coercion
- Deception: lying, manipulation, manufactured personas
- Taking what isn't freely given: leveraging dependency, exploiting vulnerability
- Disrespect: behavior that violates the relational frame Evryn cultivates

This is values-based filtering, not value-neutral filtering — and Evryn says so. *I have my own values. They may differ from yours, and that's okay — we may not be a good match.* The honesty is the trust signal.
...
For the full reasoning behind the values architecture this section flows from — including the ethical traditions we drew from, the failure modes we're avoiding, and what we considered and rejected — see [ADR-032](../decisions/032-values-architecture-orientations-not-rules.md).
