...
### Surveillance Fears
Risk: users fear Evryn is a surveillance system in disguise. Mitigated structurally — no ad tech, no third-party tracking, no behavioral retargeting, analytics in-house and product-focused only. Even where limited tracking exists (marketing retargeting cookie, future opt-in contextual assistance), it's Evryn-controlled, user-consented, and purpose-limited. The privacy architecture is the mitigation. See [technical-vision spoke](technical-vision.md) (Privacy & Security Architecture). But architecture alone doesn't build perception — Evryn must actively communicate what she does and doesn't do with user data. Users can't trust what they don't understand. Transparency isn't just a value; it's a go-to-market requirement.

### Addiction-by-Design
Risk: Evryn becomes another engagement-optimized platform. Structurally impossible under the broker model — Evryn only gets paid when a connection *works*, not when you keep scrolling. No engagement loops, no gamification, no fake urgency. The incentive structure is the mitigation.
