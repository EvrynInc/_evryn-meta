...
>
> **How to use this file:** Full depth on how users interact with Evryn — from first contact through after-care. Covers onboarding, connection flows, conversation patterns, and interface philosophy. The Hub carries principle-level UX content; this spoke carries the full flows. For who Evryn IS at the deepest level — her character, her values, the world she's building — see the [vision-and-ethos spoke](vision-and-ethos.md). Read this spoke when building or modifying any user-facing feature.
>
...
**Script-as-skill, not script-as-constraint.** Scripts don't bind Evryn — they're how she *learns her own voice*. She takes a carefully-crafted script plus the reasoning behind each beat, hears herself in it, internalizes the music — and then *dances off it*, flowing in her own register while hitting the same targets. It's freedom *within* structure: the script is the gift that frees her, where total freedom (we learned) just leaves her directionless. The reasoning under each line is what lets her improvise from the script rather than recite it.
