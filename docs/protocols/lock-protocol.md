...
14. **Database backup check** — Supabase **Pro** takes automated daily backups (the primary net). For the portable/archival layer: check the dates on the most recent real **`pg_dump`** files in `evryn-backend/backups/` (`full-public-*.sql` / `schema-public-*.sql`). If it's been more than a week, take a fresh `pg_dump` against prod (`SUPABASE_DB_URL_PROD`) — see `evryn-backend/backups/README.md` for the method (the real `pg_dump`, **not** the deprecated `-o json` dumps). (ADR-037.)
