...
3. **Spin the Runtime Scout — it IS the orchestration protocol's *reporter* AC-subagent** (`docs/protocols/orchestration/spinning-an-ac.md`: *a meta-level AC needing a map of a domain it can't afford to load itself*). Don't grep just for this section or you'll miss the supporting info — you must read the whole protocol to understand it. **Spin the subagent strictly per that protocol** — the AC-variant tagged brief, the load-gate, the model pin, and verifying its receipts on return are all the protocol's job; do NOT restate that machinery here. This section adds only what's specific to *this* use — the fixed choices, the load, and the return format:
   ...
   - **Its load, under the three-trip shape.** ⚠️ **This bullet used to hand the scout the build's entire Full-cascade file set — intended-shape docs, changelog skim and all. That is the load that failed, and it is superseded by the box above.** What it gets now:
     - **Trip 1 (DERIVE):** its identity · **the map artifact** (for `evryn-backend`: all of `docs/atlas/` + `docs/dependency-map.md`) · `current-state.md` · the task + the verbatim `<output>` block. **Nothing else.**
       > ⚠️ **Do NOT put `CLAUDE.md` on this list. The harness auto-injects it into every agent you spin**, so naming it buys nothing and invites the scout to spend a read on something already in its context. **The router's own instruction is to DIFF the injected copy against disk — not to re-read it — because the injection has been observed serving a stale snapshot.** *(Removed 2026-09-02; it had been listed here since before the router split.)*
     - **Trip 2 (LOAD):** the list *you approved*, which is normally **both runtime halves enumerated LIVE** — the code half AND the identity half **separately**, because `find src -name "*.ts"` cannot return an identity file (the enumeration trap) — plus whatever the derivation surfaced. **You assemble and name every file with its line span**; that is the protocol's part-2 discipline and it is not shortcut-able.
     - 🔴 **The active handoff and any lane brief are TRIP-2 or LATER reading, never trip 1** — see the protocol's *"THE LANE BRIEF IS TRIP-2 READING."* A brief IS the task; putting it in the load trip hands back the very judgment the trip exists to remove. *(This bullet used to list them under trip 1. That was wrong and is corrected here.)*
     - **The intended-shape docs (ARCHITECTURE, BUILD, the sprint, the changelogs) stay with YOU** unless a specific trip genuinely needs one in the scout's head. That is the trade that makes the runtime fit.
   - **Return, structured:**
