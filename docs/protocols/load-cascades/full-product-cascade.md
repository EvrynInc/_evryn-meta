...
5. **Skim `evryn-backend/CHANGELOG.md`** — read the top ~100 lines (newest-first, so that's the recent ships in full) + the `## <date>` section headers below for the shape of the rest (~30 seconds). This is what recently **shipped** to the product; `current-state` can lag it, and this keeps you from re-proposing a shipped thing or being surprised by it in the runtime.
