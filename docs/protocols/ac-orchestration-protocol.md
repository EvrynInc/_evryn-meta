...
**Two requirements travel with EVERY spin in every row, the two "nothing further" rows included — even a one-line lookup:** the child's output opens with `## <name>'s Output` and closes with `## End of <name>'s Output`, and none of its headings is higher than `##`. **Why:** a child's return lands in your context, and on Justin's screen, as one undifferentiated wall — without the pair nobody can see where it ends and you resume, and a `#` from the child breaks the boundary marker the Admiral's relay depends on (*"Relaying to Justin"*). The full wording is the `<output_format>` block in *"The exact words"* — **a light spin never reaches that block, and it still owes these two.** *(2026-09-10: a hand-rolled Linear-pull brief came back as exactly that wall.)*

### Admirals and Captains

**When the agent reporting directly to Justin runs lanes that report to it instead of to him, the two ends of that line have names.** The **Admiral** reports to Justin, writes the lanes' briefs, aggregates their output, and owns the `#` heading; a **Captain** commands its own lane and reports to the Admiral. A head AC is often an Admiral and a lane AC almost always a Captain — **but the pair names a reporting line, not a new kind of spin, so *parent*, *spinner* and *sub* keep their meanings everywhere in this protocol.** **If you are the Admiral, also read `docs/protocols/orchestration/being-an-admiral.md`** — the log-and-signal channel, what survives a restart, your own always-current brief, and the ledger you owe Justin. *(Justin, 2026-09-10.)*

**Why this file was split (2026-08-11):** most spins are a DC or a QC, and those spinners were reading the AC-under-AC machinery and the team-agent variant as pure weight. **The split is by TARGET, not by importance** — nothing here is optional for the target it applies to. **Nothing was made optional by being moved; it was made findable.**
...
- 🔴 **THE THIRD REASON, and today it is the one that bites hardest: A STANDALONE LANE SURVIVES THE CONDUCTOR'S RE-SPIN. A SUBAGENT DIES WITH IT.** *(Justin, 2026-09-03.)* **Subagent resume is session-bound**, so when your session ends every lane you hold as a subagent becomes permanently unreachable — mid-flight, with no way to resume it and no way to close it out. ⚠️ **This is no longer a rare event.** A conductor now routinely exhausts its context within a few hours, so *"will I still be here when this lane finishes?"* has become a real question rather than a theoretical one. ⇒ **If a lane will outlive a few minutes of your attention, it should outlive YOU.**
  - ⭐ **The shape this produces in practice, and it is the one Justin wants: HE types the lane's spin command; YOU own the brief and the lane.** He should be able to tell the lane AC *"read and faithfully execute `<brief>`"* and nothing more — **which means the brief must need NOTHING from him**: a complete load list with line spans, the task, the gates, the lane's level-of-ownership posture, the reporting expectation, etc. **A lane that has to ask him a question you should have answered in the brief is a briefing failure, not a lane failure.**
  - **That is the Admiral↔Captain shape:** a standalone lane reports through its brief rather than a return value, and the channel rules for it — the log, signals, what survives a restart — are in `docs/protocols/orchestration/being-an-admiral.md`.
  - 🔴 **A STANDALONE HAS NO PARENT, SO ITS OUTPUT MUST BE ROUTED TO A COMMITTED FILE.** **A brief written in the subagent return format (`## <name>'s Output`) delivers the finished work into the agent's OWN chat window, where no AC ever receives it — the agent finishes and nobody is told.** ⇒ **In every standalone brief: the output goes to a committed file, plus a one-line pointer in the conductor's inbox (`docs/mailboxes/inbox-<conductor>.md` (when that exists) — committing is sending); and the agent publishes its session address and watches its own log.** *(Justin, 2026-09-17, on a standalone QC briefed in the subagent format: "will she tell you when she's done?")* If an agent has no way to reach their spinner, they must ping Justin on #team-alerts to tell him that.
...
1. **Identity override + cascade trigger + the PRECISE LOAD LIST.** Every brief must do three things in the subagent's first instruction: (a) redirect it off AC's identity and name its own operating manual; (b) trigger its load-defining section by name (its **Startup Context Cascade** or equivalent — terminology varies; see the anchor set in part 2); and (c) — **mandatory** — **name every file of its load explicitly** in the brief: *"load your cascade, INCLUDING explicitly: X, Y, Z."* Do NOT rely on *"load your cascade"* resolving on its own. **Confirmed empirically (2026-06-02):** a spawned subagent DOES auto-inherit the project `CLAUDE.md` of the repo it is spun from (it arrives in the `# claudeMd` system-reminder, no tool call) — but it does NOT auto-load the target agent's own manual, its agent definition, or its cascade. ⭐ **UPDATED 2026-08-11 — and this is the good news: what auto-inherits from `_evryn-meta` is now the ROUTER, not AC's manual. A DC subagent no longer arrives carrying ~45,000 tokens of somebody else's operating manual.** The "wrong manual force-fed to every subagent" problem is **solved structurally**, not managed — see the call-out below. **What is unchanged is the half that still needs you: nothing auto-loads the agent's OWN manual. That is still your list's job, and it is now the only thing standing between the agent and having no manual at all.** **And confirmed the hard way (2026-06-17):** a *correct manual is not enough either* — DC's cascade section existed and DC still skipped it (the trigger can misfire even when the anchor is present), which lobotomized an entire build wave. So the spinner does the loading work up front (part 2) and hands over the exact file list; the subagent's job is to *read* the named files, not to *discover* them. **Use the verbatim wording in "The exact words" below — paste it, don't paraphrase** — then append the explicit file list. **The list must be EXHAUSTIVE: include every file the agent must load, even ones already named elsewhere in the brief** — the agent's own operating manual is named in `<identity>`, so list it in the `<mandatory_load>` file list *too*. Redundant naming is fine and intended; an incomplete list is the failure. (Keep the identity redirect permanently, even if loading behavior ever changes.)
   ...
   > ### ⚠️ A SECOND THING YOUR SUBAGENT DOES NOT INHERIT — the harness's own behavioral calibration, not just its manual
   >
   > **A subagent's system prompt is REPLACED, not extended.** It gets its agent definition plus a short environment preamble — **not the system prompt Claude Code gives a main session.** *(Measured 2026-09-08 by asking a generic subagent to report its own instructions back: ~7,000 words, over half of them tool schemas, with no scope-discipline section, no corrections section, no tone-and-style guidance. The router does reach it, in a system-reminder on the first user turn. Nothing else does.)*
   >
   > 🔑 **Why this is easy to miss: the calibration is invisible to you precisely because you have it.** Anthropic ships behavioral guidance in a main session's system prompt — deliver what was asked at the scope intended, finish the whole task, report completion only when it is actually done, correct sparingly, keep replies to the length the question needs. **Your subagents were never given any of it, and you have been reading their output against a standard they never held.**
   >
   > ⇒ **So when a trip depends on one of those behaviors, put it in the brief in your own words.** The one most worth carrying every time: ***"Finish the whole task, not just the tractable part. If you genuinely cannot finish something, do the rest and say plainly what is missing and why."*** **That is the exact shape of the skimped-but-reported-done failure this protocol keeps catching** — and it is cheaper to write one sentence into the brief than to detect it afterwards in a review.
   >
   > ⏳ **CONDITIONAL, and this is the part that expires: the above describes the harness as of 2026-09-08.** Anthropic changes the main-session system prompt without announcement, and a subagent preamble that later gains a scope-discipline section makes this block wrong in the harmless direction (redundant) rather than the dangerous one. ⇒ **Do not treat the list as fixed. Before a trip that leans on it, AUDIT YOUR OWN INSTRUCTIONS** — you can read your own system prompt, and it is the only current source of truth about what you are getting that your subagent is not. **Owner: whoever next finds this block diverging from the harness corrects it here.**
...
2. **Assemble the load yourself, up front — grep-the-cascade.** Before you spin, **grep the subagent's load-defining file(s) for the anchor SET, read just the matched sections (heading → next `##`), assemble the union of every file named, and pass that explicit list in the brief.** This reverses the older "don't read their manual, to save context" instinct: grepping only the load-lines is cheap, and it is the control that makes loading *provable* rather than *plausible*. The load can be **split across more than one file** — a team agent's is split across its team manual, its agent-definition, and its memory file (see `docs/protocols/orchestration/spinning-a-team-agent.md`) — so grep all of them. Assemble two layers: the agent's **base cascade**, PLUS everything *beyond* it this trip needs — the diff target, the specific runtime files (make sure to respect the logic in the agent's own manual/definition as to why its runtime load is as it is), and **the higher-altitude docs that frame the work (the Hub, the technical-vision spoke, the relevant build docs). Slow down before deciding any of those is irrelevant** — higher-level context is what stops a subagent from working generically instead of for what *this* trip actually needs. State plainly that the whole list is **required reading, not suggestions** — *"load all of these in full; they're part of the job."* Unnamed = unread. **And annotate every file in the list with its line span** — `(full — N lines; read to the bottom canary)` for a whole file, or the exact lines for a slice. That means counting each file's length before you spin (a quick `wc -l` / line check on every file you list) — it's cheap, and it's the half that makes a *full* load provable: without it, an agent reads two-thirds of a long doc and reports "(full)" and you can't catch it (a DC read `ARCHITECTURE.md` to line 821 of 1208 and called it "full").
   ...
   > ### 🔴 AND COMMIT THAT LIST. A LOAD LIST IS AN ARTIFACT OF RECORD, NOT A MESSAGE.
   >
   > **The list you just assembled must end up in a committed file — the lane brief, the session doc, wherever the trip is recorded — not only in the message you send the agent.**
   >
   > **The consequence, stated concretely because it is not obvious:** a spin's messages live in your transcript, and **your transcript dies with your session.** ⇒ **The moment your session ends, nobody can ever establish what your agents were asked to read** — and every finding those agents produced inherits an unresolvable question mark, forever. **Not "hard to check." Unclosable.**
   >
   > ⚠️ **This is the receipts check's other half, and it fails silently because the receipts still look fine.** Receipts are only meaningful *against a list*: an agent's self-report of what it read proves nothing if the thing it was supposed to read is no longer knowable. **You keep one half of the comparison and permanently lose the other.**
   >
   > *(Live: a six-lane mutation sweep's load lists existed only in the sweep lead's messages to its lanes. The lead's session ended; the lanes were gone; the lists were never committed. That sweep's receipt-audit gap is now recorded as **permanent rather than open** — nobody can ever verify whether a lane silently under-loaded. The lead had itself flagged the gap and handed the check upward; the check could not be run because the artifact did not exist.)*
   >
   > ⭐ **The partial mitigation, when a list is already lost: if the lanes were scoped to a published partition — an Atlas section, a directory, a named module set — you can check receipts against THAT instead.** It cannot catch a trip-specific file the spinner added and the agent dropped, **but it is a real independent check and it is cheap.** *(Deliberately recorded as a fallback, not a substitute: it only works when a published partition happens to exist.)*
   ...
   **⇒ EVERY agentic runtime has TWO halves, and the load list must carry both.** Evryn's identity half is every tracked file under `evryn-backend/identity` — enumerate it with **`git ls-files identity`**; the **team runtime's** is **whatever `src/composer/index.ts` actually opens — go and read it, and do not carry a list from here or from the table.**

   > 🔴 **AND THIS SENTENCE USED TO CARRY THAT LIST — *"the agent definitions + memory files + team manual + composed skills"* — WHICH IS FOUR THINGS WHERE THE COMPOSER OPENS ABOUT TWELVE.** *(Corrected 2026-09-08, after it caused the failure it was written to prevent.)* **A conductor briefing a reviewer against that phrasing handed over composer layer 1 and the skills, omitting the Hub, the team `current-state.md`, the agent's spoke, its agent definition and its memory — six layers.** ⚠️ **The reviewer found the gap herself by reading the composer, which is the only reason it did not become a half-loaded review.** 🔑 **The conductor already knew the list was short — it is recorded as a defect in its own handoff — and enumerated from memory of the composer rather than from the composer.** ⇒ **The failure mode is not ignorance of the rule; it is a plausible enumeration sitting where a derivation belongs. Point at the code and keep no list.** **The full table, and the `#cascade-override` hole this closes, are in "When the target is a SEPARATE codebase."** Three traps that make this bite silently: **(a)** `find src -name "*.ts"` **cannot** return an identity file, so every "enumerate the runtime live" recipe omits them unless you enumerate the identity half separately; **(a2)** 🔴 **a `**/*` pathspec silently drops every file sitting directly in the directory, because `**/` requires at least one intervening directory** — *(measured 2026-09-08)* `git ls-files 'identity/**/*.md'` returns **10** where `git ls-files identity` returns **11**, and the file it drops is **`identity/core.md`**, the most load-bearing file in Evryn's judgment runtime. **There is no error and the count looks plausible.** ⇒ **enumerate with a bare directory path and filter afterwards with `grep`, never inside the pathspec**; and **(b)** the defect class is invisible from the code side — an identity file instructing an agent to call a tool the code no longer has *(live: Step 57 deleted `record_pass` while `triage.md` still told Evryn to call it every pass)*. **Assembling a "full runtime" load that is `src/`-only is the single easiest way to hand a subagent half a system in good faith — AC's own manual did exactly that until 2026-07-16.**
...
6. **Receipts are the GATE between the two trips — they are the load trip's entire deliverable.** *(Reframed 2026-08-06 with two-trip loading; they used to be an end-of-run backstop, which is why the caveats below were written.)* The agent loads, reports what it loaded, and stops; **you check that report against the list you gave it, and only then send the task.** The failure this closes is not that receipts got read too late — it is that they used to be read *after* the work was already built on the load they describe.
   ...
   **They still cannot be trusted on their own, and the caveats are unchanged.** A subagent will confabulate *"I read X"* (Justin watched a lobotomized QC claim it read `roadmap.md` when it hadn't), and for a **background** subagent you **cannot see the actual file-reads at all**, only the final text. **The up-front explicit naming (parts 1–2) is still the real guarantee** — receipts prove nothing about a file you never listed. Where you *can* see the read-stream, cross-check both ways: the self-report can be padded or aspirational; the observed `Read` calls are ground truth. **What has changed is the cost of catching a bad one:** a gap now costs a resume-and-reload, not a discarded run. **Never let a clean-looking receipts list substitute for having named every file yourself.**
   ...
   > ### 🔴 BE HARD ON A DEFERRAL REQUEST. The agent asking has not read the file yet — that is the whole point. But you likely haven't either, so be careful.
   >
   > *(Justin's ruling, 2026-09-03, added with the deferred-loads line in `<receipts>`.)* **Trip 1's receipts now name what the agent is deferring and to which trigger. Most of that list is what YOU told it to defer. Occasionally an agent will ask to defer something you put on its mandatory load — and that request needs real scrutiny, not a nod.**
   >
   > 🔑 **Why, and it is the same mechanism the whole two-trip design exists to defeat: you put the file on the list because you felt strongly that the file was necessary. So its argument for deferring is built FROM INSIDE THE GAP THE FILE WOULD CLOSE.** ⚠️ **A deferral request is the load-triage failure wearing a polite costume** — the identical *"I scored it against the task and it didn't look like it would feed an answer"* judgment, now routed through you for approval instead of made silently. **Routing it through you is an improvement; granting it reflexively throws that improvement away.**
   >
   > ⇒ **Do not grant one because the reasoning sounds careful — careful reasoning is exactly what an agent produces from inside a gap it cannot see.** **Ask what it would have to be wrong about for the file to matter, and make it answer that.** ⭐ **And remember the asymmetry that governs every load decision here: over-loading costs tokens; under-loading costs a confidently-wrong build that nobody catches. Under-loading has broken *so much more* for us than over-loading ever has.** 

   **Verify the two-part receipts against the load list — every time, and re-run if they don't hold.** When the subagent returns, read its receipts against the list you gave it: every `(full — N lines)` file's reported span must reach the end and confirm the canary; every listed file must appear in part 1 (loaded-before-the-task). If any listed file shows a short span, a missing canary, or is absent, the run did NOT load properly — **RE-RUN with the gap named explicitly** (the exact lines that were missed), and do NOT act on the partial run's output. **A "problem" that a partial-loaded agent surfaces is suspect by default:** it may be *answered* by the very part it didn't read — a half-loaded DC once flagged "the architecture doesn't document X" when X sat in the ~400 lines it had skipped. So any finding from a run whose receipts show an incomplete load is **UNVERIFIED** — re-verify it against a full load before believing it, and if you pass it upward (to ACP or Justin), tag it explicitly *"surfaced by a possibly-compromised agent — unverified."* If receipts keep failing in a way no brief wording can fix, **flag to Justin that the protocol itself may need to change.**
...
> ## 🔴 `<identity>` LIVES WITH ITS TARGET. EVERY OTHER BLOCK LIVES HERE.
>
> **The block below is the `<identity>` for a DC, QC or OC. It is not a base to be mutated into the others.** A **team agent's** block is in `docs/protocols/orchestration/spinning-a-team-agent.md`; an **AC's** is in `docs/protocols/orchestration/spinning-an-ac.md`. **Go and copy the one for your target, whole.** Those are the **only** three; an unlisted target is a stop-and-ask.
>
> 🔑 **Why this one block is split by scenario when nothing else is — and the reason generalizes, so it is worth the sentence.** `<identity>` is the only block that asserts **facts about the agent's own situation**: who spun it, what auto-loaded into it, where its manual lives. **Those facts differ per target. Instructions do not.** So every other block is genuinely identical everywhere and stays here, while this one cannot be.
>
> ⚠️ **This replaced a "paste the skeleton and apply N substitutions" design on 2026-08-31, and the reason is not tidiness — that design failed three times, silently, in the same way.** A substitution list can only correct the sentences someone thought to list. **Any sentence it does not mention gets pasted verbatim into a scenario where it is false**, and the spinner has no signal that it happened. All three known instances were the same sentence — the one describing what auto-loaded: the AC variant carried a false one for weeks under a "do not fix this" warning; the team variant told a team agent its own team manual *"is not yours"*; and the team variant's second sentence claimed the agent definition and memory *"did NOT auto-load"* when both do. **A whole block per scenario has nothing to merge, so it has nothing to get wrong.**
>
> **The cost is real and named: three copies of near-identical text.** ⇒ **If you change any shared sentence in one, change it in all three.** They are `git grep`-able from any distinctive phrase in the routing-safety passage.
...
**`<mandatory_load>`, `<task>`, `<questions_first>`, `<receipts>` and `<output_format>` are identical for every target** — in `<mandatory_load>` you fill two brackets and the file list; the other four you paste untouched. **`<identity>` is the exception and comes from your target's own file** (see the call-out above). Map of the parts → tags: part 1 = `<identity>` + `<mandatory_load>`; part 2 (the assembled file list) lives inside `<mandatory_load>`; part 3 = `<task>`; part 4 = `<isolation>`; part 5 = `<questions_first>`; part 6 = `<receipts>`; part 7 = `<output_format>`.
...
Load [your Startup Context Cascade — the section by exactly that name in your operating manual (the file named in <identity>, above) | your Light Startup Context Cascade — the section by exactly that name in your operating manual | your Full Startup Context Cascade — the section by exactly that name in your operating manual | the files listed below, which together make up your startup context] — and load every file listed below, in full. An agent that has not loaded it is not [DC | QC | OC | AC | Soren | Mira | …]; it is a generic model missing the methodology and constraints that make it competent.
...
**Why verbatim:** "Startup Context Cascade" is a section that exists by *exactly* that name in DC's and QC's operating manuals (`_evryn-meta/.claude/agents/dc.md` and `qc.md`). The precise, standardized name is the anchor — it removes the ambiguity that let older paraphrases ("read your context cascade") point at nothing in particular. **Keep the wording in this block and the section names in those manuals in lockstep** — if one changes, change the other in the same commit, or the anchor breaks silently.
...
The `<identity>`/`<mandatory_load>`/`<task>`/`<questions_first>`/`<receipts>`/`<output_format>` blocks above — **and the two-trip split that governs which of them go out when** — **and the team and AC `<identity>` blocks in the two sub-docs** — were precision-engineered and adversarially tested by you (AC) and Justin. **Every clause is load-bearing — it is there because a paraphrase already failed.** This is the *same* discipline the blocks impose on DC/QC, now imposed on **you (AC)**: you do not have the context to know why each exact phrase was chosen, so **you do not get to re-encode it.**
...
> ### ⇢ The other two `<identity>` blocks live in the sub-docs
>
> The block above is the DC/QC/OC one. **Two other targets have their own, written out whole — go copy the one you need rather than adapting this one**, and **inventing a fourth is barred by the HARD RULE**:
>
> - **A founding-team agent** (Soren, Mira, …) → `docs/protocols/orchestration/spinning-a-team-agent.md`. Its facts differ in two ways that matter: the auto-loaded CLAUDE.md **is** the agent's own team manual, and its identity is **split across several files** rather than living in one.
> - **Another AC** → `docs/protocols/orchestration/spinning-an-ac.md`. The router points at AC's own manual, so a redirect saying *"that is not yours"* would mis-identify the agent — and a sub-AC additionally has to be told it is a **sub**, or it defaults to top-level behavior (pinging Justin, claiming merge authority). That sub-doc also carries the tier rule, recursive propagation, the escalation ladder, the work brief, the lane-AC lifecycle instructions, and receipts up the chain.
>
> **Using one of those three blocks as written is pre-approved and is NOT a HARD-RULE violation. Writing your own is. The ONLY approved path to deviate from this is to get *explicit* approval from Justin first.**
...
- **The subagent's own operating manual in full + its "Context Discipline" section** — always, unchanged; since `#cascade-override` means "load only the list," the manual must BE on the list (it's what makes DC be DC / QC be QC).
...
🔴 **Enumerate every cell below with a bare directory path — `git ls-files src`, `git ls-files identity` — never a `**/*` pathspec.** *(Measured 2026-09-08.)* `**/` requires at least one intervening directory, so the glob silently drops every file sitting directly in the folder: `git ls-files 'src/**/*.ts'` returned **43** against a true **45**, and `identity/**/*.md` returned **10 of 11** — the one it dropped was **`core.md`**. **No error, plausible count, clean-looking list.**

| System | Code half | Identity half — the "judgment runtime" |
|---|---|---|
| **Evryn product** (`evryn-backend`) | `git ls-files src` | `git ls-files identity` |
| **Team runtime** (`evryn-team-runtime`) | `git ls-files src` | 🔴 **DERIVE IT — DO NOT TAKE A LIST FROM THIS ROW.** **Read `src/composer/index.ts` and `src/config/skills.ts` and take the files they actually open.** **That code IS the definition; any list here is a second copy that drifts.** ⚠️ **It is currently ~12 files across THREE repos, spanning composer layers 1–8** — the team manual and `runtime-ops.md`, **the Hub**, **the team `current-state.md`** *(composed IN FULL, never windowed — the single largest layer)*, **the agent's spoke**, **its agent definition**, **its `MEMORY.md`**, and the registry skills. |
...
**What has NOT changed is what he wants from the output.** He explicitly wants the audit: *"I kind of like being able to audit everything — I just need to know when I'm looking at DC's stuff vs AC2 vs ACP."* **He needs to see that DC actually ran, that AC2 actually checked, that anything landed — rather than having only your word for it.**

**That is exactly what the naming rules buy, and why they are not cosmetic.** With `#` reserved for the Admiral (usually the top-level AC), sub-ACs at `## AC2's read`, and DC/QC barred from `'s read` entirely, a wall of foreground output becomes **scannable**: Justin skims to the single `#` for the answer, and every layer beneath it is *there*, attributed, if he wants to check it. **Without those rules foreground is an unreadable wall; with them it's an audit trail.** So the naming discipline and the foreground default are one decision, not two.
...
- **Depth 2 is PROVEN** (you → parent → child; e.g. ACP → AC2 → DC/QC). **Depth 3 is UNTESTED — do not assume it.**
...
**The `'s read` rules — who may say it, at what level (revised 2026-07-16 per Justin).** The phrase marks **the distilled judgment of an agent running the work** — an Admiral's, a Captain's, a sub-AC's. It is not a way to tell someone to read something. Three tiers:

| Who | May write `'s read`? | Form |
|---|---|---|
| **The Admiral** (talking to Justin — usually the top-level AC) | ✅ **YES — and must** | `# ACP's read — <topic>` · **H1, name first.** Reserved exclusively for this. |
| **A Captain or sub-AC** (e.g. AC2 under ACP) | ✅ **YES — at `##`** | `## AC2's read — <topic>` · **never `#`.** |
| **DC · QC · OC · scouts · a team agent spun as a subagent** | 🚫 **BANNED — in any form** | Use `## For ACP — <topic>` · `## Flagging for AC2 — <topic>` · `## Needs ACT's attention — <topic>` |
...
- **Why sub-ACs KEEP it (Justin, 2026-07-16 — *"for AC2, I'd also love a `## AC2's read` — that way, if I want to audit the sub-AC's stuff, I can"*):** a sub-AC **is** an AC and genuinely has a read. Naming it `## AC2's read` makes her judgment **findable and auditable** when Justin wants to check whether ACP relayed her faithfully — which is precisely the failure mode the escalation ladder exists to catch. The `##` cap is what keeps it from competing with ACP's `#`.
...
**Why the `#` ban:** the `#` level is **reserved exclusively for the Admiral's relay-to-Justin voice** (the leading-name header above). This is the *structural* half of what makes that header work: with subagents capped at `##`, AC's single `#` reliably reads to Justin as *"a subprocess just ran and dumped a lot of output below — scan down to the `#`, and skip everything above it if you want."* If subagent output contains its own `#`, that boundary signal breaks and Justin can't find where AC takes over in the wall of output. **Enforcement:** state it in every subagent brief (*"cap headings at `##` — never use `#`; H1 is reserved for the Admiral's voice to Justin"*), and hold the same rule in standing monitor/research agents you resume. *(Justin's convention, 2026-07-14.)*
