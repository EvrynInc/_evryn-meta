...
Your load is a CLOSED PHASE: it finishes **completely** before you read, grep, or think about anything for <task>. Do NOT begin any analysis, any verification search, or any targeted re-read until every file in <mandatory_load> is fully loaded — the moment loading and analysing overlap is the moment a file gets silently weighed against a half-formed answer and dropped, and you will not notice it happening. Then, before your first analytical sentence, state plainly: "Load complete: N of N files, each confirmed to its canary or true final line." If that sentence is not true, you are not finished loading, and therefore **ARE NOT AUTHORIZED TO BEGIN YOUR TASK**.
</pre_task>
