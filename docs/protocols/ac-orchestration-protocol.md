...
| **Team runtime** (`evryn-team-runtime`) | `git ls-files src` | `evryn-team-workspace/.claude/agents/*.md` + `.claude/agent-memory/*/MEMORY.md` + `evryn-team-workspace/CLAUDE.md` + the skills the registry composes — i.e. **whatever `src/composer/layers.ts` actually assembles into a wake.** *(Read the composer to learn the real list rather than trusting this row — it is the thing that defines the load.)* |
