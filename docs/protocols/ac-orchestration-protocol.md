...
- **A `SendMessage` you send to a child arrives with `from` set to the agent TYPE, not an ID** (observed 2026-08-04: a DC received a fix-trip message showing `from=general-purpose`). So a child **cannot** address you back even when it would be useful, and it is *correct* for it to refuse to try — that is the role-vs-literal rule working as designed. Auto-return covers the case; **don't design a flow that depends on a child replying to you directly.**
