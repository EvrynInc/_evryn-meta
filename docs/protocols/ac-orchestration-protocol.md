...
<task>
⚠️ **LOAD-COMPLIANCE — READ THIS BEFORE YOU START THE TASK.** Across past spins, you (the DC/QC role) have *repeatedly* reported a file as "loaded" when you had only PARTIALLY read it — you read the first chunk of a long doc and judged the rest irrelevant. **You could not have known it was irrelevant; you hadn't read it.** Every single time, it has either broken something or forced an expensive re-spin to redo the work with the file actually loaded — the single most common and most costly mistake you make. So before you touch anything below: go back over `<mandatory_load>` and confirm you read EVERY file IN FULL — each long doc all the way to its final line and its `FULL FILE LOADED` canary, not two-thirds and called done. Your receipts must show the exact line range for each, and any `(full — N lines)` file's range MUST reach the end. **Do not be the next instance to skip or partial-load a document you were told to load in full and make us spin you all over again.**
