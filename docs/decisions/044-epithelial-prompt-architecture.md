...
2. **The churning `pending_notes` move OUT of the `systemPrompt` and into the first user message**, clearly delimited and labeled as Evryn's own trusted observations (distinct from the untrusted incoming email, which is also in the user message). The raw incoming email stays in the user message exactly as today.
