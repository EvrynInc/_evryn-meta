...
> **📌 Quotation-accuracy footnote (added 2026-07-29, Soren — additive only; no decision text below was changed).** A requirement below states that **`core.md` must include** *"You never send anything to anyone without **operator** approval. Every outbound message goes through the approval flow. No exceptions."* **The line that actually shipped is approver-agnostic:** *"You never send anything without approval. Every outbound message to a user or external recipient goes through the approval flow. No exceptions."* (`evryn-backend/identity/core.md` §Hard Constraints). The requirement text is left as written — it correctly records what this ADR *specified* in March 2026 — but **do not quote it as the current identity text.** The distinction matters because the constraint binds Evryn's own conduct rather than the approver's identity, so the approver can change (**The Publisher — Justin during v0.2, the Publisher module in v0.3+**) without amending it. Canonical statement: `evryn-backend/docs/ARCHITECTURE.md` §"Outbound Approval Gate (Architectural Invariant)."
