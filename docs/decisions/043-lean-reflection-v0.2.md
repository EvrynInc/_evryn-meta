...
- **Tracked hardening (non-blocking, before enablement) — SPRINT Steps 71–72:** (a) a **binding structural safety-net** so consolidation never silently clears a note bearing a live `[binding]` that didn't make it into the new story (Mira's flag, 2026-06-23 — the identity files instruct Evryn to carry it, and the live test showed her doing so, but it's LLM-dependent and a dropped binding is a broken promise; the guard is belt-and-suspenders); (b) pin `SET search_path` on all three SECURITY DEFINER RPCs (`consolidate_profile` + `append_pending_note` + `append_cross_user_note` — all share the gap; not introduced here) + a small `_meta`-missing robustness tweak. A v0.3 soft-edge: the validation guard doesn't catch a plausible-length-but-wrong-content output (e.g. a leaked preamble) — recoverable via the archive at v0.2, harden at v0.3.
- **Soft-target tuning lever (Mira, 2026-06-23):** `reflection.md` gives Evryn a *felt* soft-target ("take in at a sitting — a couple pages / a few lines"), deliberately NOT the literal `REFLECTION_STORY_SOFT_TARGET_CHARS` (env-tunable → hardcoding a number into an identity file goes stale). The live test landed ~2,900 chars (on target). If consolidated stories ever drift in length, inject the literal number into `buildConsolidationPrompt` (where the constant lives), not into the identity file.
