...
> **📌 Quotation-accuracy footnote (added 2026-07-29, Soren — additive only; no decision text below was changed).** This ADR paraphrases Evryn's tier-5 outbound constraint as *"never send (anything) without **operator** approval"* in five places (Context, the axis table, "permanent" rules, identity-weight, and the tier-5 example). **The line that actually shipped is approver-agnostic.** `evryn-backend/identity/core.md` §Hard Constraints reads: *"You never send anything without approval. Every outbound message to a user or external recipient goes through the approval flow. No exceptions."* — no "operator" anywhere in it. **That distinction is load-bearing for this ADR specifically:** because the constraint binds *Evryn's own conduct* rather than the identity of whoever approves, replacing the approver — **The Publisher (Justin during v0.2, the Publisher module in v0.3+)** — does **not** relax, transition, or amend the tier-5 rule. That is precisely what this ADR asserts at §"Publisher as backstop at tier 5," so the correction supports the decision rather than disturbing it. The paraphrases are left exactly as written: an ADR records what was decided, and editing its body falsifies the record. Canonical statement of the invariant now lives in `evryn-backend/docs/ARCHITECTURE.md` §"Outbound Approval Gate (Architectural Invariant)."
...
The runtime-bookkeeping rewrite ([ADR-051](051-runtime-bookkeeping-structured-verdict.md)) applies this ADR's **tier-5 "prefer structural enforcement"** principle to Evryn's database *write* surface — the write-side sibling of the read-lock in [ADR-047](047-v0.2-security-bulkheads.md).
