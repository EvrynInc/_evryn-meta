...
## Update — S2 / review@ live-fire additions (2026-06-10)

All trace to one operator-error seam (Justin replied to a draft **from `review@evryn.ai`** instead of as Mark), but each is a real runtime/identity gap the seam exposed. **Nothing reached the contact** (approval gate held; 0 messages to Garrett; the Garrett rejection draft was reviewed, refined, then killed as testing). Buckets as before: **[1]** before invite · **[2]** week-of-6/8 · **[3]** v0.3.

- **Finding 13 — Internal/operator addresses (`review@evryn.ai`) are processed as new external users. [2/3 — cheap fix; trigger is operator-only]**
  The runtime self-skips only `evryn@evryn.ai` (`config.gmailUser`). Replying from `review@evryn.ai` → `findOrCreateUser` created a phantom user (`611d9280`, "Justin as Reviewer"); `detectForward` saw the quoted "[Evryn] Edge Case" review body and ran `processForward` under the phantom's scope → spawned items (`41a1`, `7908`) + a draft to Mark off the reviewer's text. **Root cause of the whole S2 cascade.** `review@evryn.ai` is internal plumbing (the format-review BCC target), not a sender. *Fix:* a small internal-address denylist in the inbound guard (`handleNewEmail`/`processEmail`) — skip `review@evryn.ai` (and any internal alias) the same way `evryn@` is self-skipped. Real-world likelihood is low (only the operator can send from review@; Mark forwards, never replies from review@), but the fix is trivial and prevents a genuine attribution mess.

- **Finding 14 — No triggering-sender anchor → attribution confusion. [3]**
  Because the phantom review@ inbound and Mark's real "pass" response landed close together, and the phantom was processed as real mail under its own scope, Evryn had two parallel "Mark rejection" triggers and could not tell which drove her response — across Slack she variously attributed it to "Mark," to "you" (Justin), and conflated the two. *Cause:* the runtime doesn't hand Evryn an explicit, unambiguous "this is the exact message + sender you are responding to" anchor; once a phantom muddied sender identity she lost the thread. *Fix direction:* pass the triggering sender/message identity as explicit context (same family as runtime-owns-context / runtime-owns-history). Finding 13 removes the proximate cause; this is the deeper fix.

- **Finding 15 — In an operator conversation she posts BOTH in-thread and out-of-thread (duplicate). [2 / Mira]**
  During the recovery she replied in-thread (via `say` in `handleOperatorMessage`) AND separately called `notify_slack`, which posts a NEW top-level null-scoped message — producing the near-duplicate "in the thread and out of the thread" responses. Not a feature. *Fix:* in an operator conversation the in-thread reply IS the response; `notify_slack` is for proactively surfacing things, not echoing her own reply (Mira beat; optionally gate `notify_slack` exposure out of `handleOperatorMessage`).

- **Finding 16 — She fabricated an `about_user_id` UUID. [2 Mira / 3 runtime-guard]**
  Her own admission (null-scoped ping): *"I passed an about_user_id UUID that I'd fabricated."* Under the phantom confusion she invented a user id rather than omitting it. The `notify_slack` tool only validates UUID *format* (`z.string().uuid()`), so a well-formed fake passes and scopes a message to a non-existent user. *Fix:* Mira — never fabricate identifiers; omit if unknown. *Runtime guard (v0.3):* validate `about_user_id` against an existing user before accepting the scope.

- **Finding 17 — Operator-profile working-knowledge doesn't load in the draft pathways (the learning gap). [3 / architecture note — drives the Mira "don't self-persist general drafting notes" beat]**
  Evryn writes calibration notes to the Operator's `profile_jsonb.pending_notes`, but `loadOperatorSuffix` runs only in operator-audience pathways (`handleOperatorMessage`, cron, `handleRevisionNotes`) — NOT in `processForward`/`processDirect`. So a general drafting/voice calibration she writes there is in context when she REVISES a draft (notes path) but NOT when she GENERATES a fresh draft off an inbound. *Implication (the durable fix):* general drafting calibration belongs in identity files (which load everywhere), not Operator-profile pending_notes; Evryn should flag such calibration to the Operator for codification rather than self-persisting it where it won't load. See the paired Mira beat (S2 item 16). Confirmed live: she did NOT write to her own profile (correctly — empty `[]`); only the Operator's.

*Source: S3b live-fire 2026-06-09 (Findings 1-11) · S1 2026-06-10 (Finding 12) · S2/review@ 2026-06-10 (Findings 13-17). All verified against prod (Evryn Product / West Coast) via psql + Railway tool-call logs.*

---

## Update — S7/S8 additions + open questions (2026-06-10)

S7 (Rick) = correct silent clean-pass (well-reasoned name-drop resistance). S8 (Amy) gold flow worked cleanly (gold → matched, Mark approved, **nothing sent to the real Amy**, warm-intro lever correctly recognized — she framed Amy as "the warm intro itself, not someone to evaluate against the three pillars"). The threads around both surfaced:

- **Finding 18 — Note verbosity is a real cost + effectiveness problem at scale. [2 / Mira + 3 / architecture]**
  Per item, Evryn writes multiple long notes — Rick (pass): a ~400-word note to Rick's profile AND a ~600-word "five-zone map" note to Mark's profile; Amy (gold): the draft + profile notes + a long inference-discipline note to the Operator profile. At Mark's ~6,000 passes/month (plus golds/edges), the output-token cost alone is material — and much of the writing is **wasted**: Operator-profile notes don't load at draft time (Finding 17), and nothing consolidates `pending_notes` in v0.2 (Reflection is v0.3). Part of *why* she over-writes: she's compensating for the absence of Reflection — preserving everything for "future me" because nothing will distill it. *Fix:* (a) Mira voice beat — notes must be **clear but concise** (bar = "a future instance can act on this," not "a complete record"); (b) v0.3 Reflection consolidation removes the defensive-over-writing pressure.

  **Pass-note policy (Justin, 2026-06-10) — the default for a pass on the gatekeeper's account is ZERO profile notes.** The item's `triage_result` + `triage_reasoning` columns already record the decision; any conversation lives in `messages`; the *why* is **re-derivable any day**. So a pass needs no `pending_note` to the contact's profile OR the gatekeeper's — and never a cross-contact map. At most a *one-line* note when there's a genuinely non-re-derivable signal (consider a hard char cap). Keep `triage_reasoning` itself tight too. This is the sharpest "don't hoard for future-me" — she has the messages + the item; she shouldn't preserve reasoning she can reconstruct. The big v0.2 cost lever, since passes are the silent majority (~6,000/mo). → Mira beat.

- **Finding 19 — Cross-contact calibration intelligence is being written into a single user's profile. [3 / v0.3 evryn_knowledge + Reflection]**
  The Rick note wrote a cross-contact "five-zone map" (naming Devin, Garrett, Nadia, 42-Justin and their dispositions) into **Mark's** (the gatekeeper's) profile. That's Evryn's own cross-user *calibration* intelligence — which architecturally belongs in `evryn_knowledge` (global patterns) / her working intelligence, NOT in a user's profile (it bloats the profile and crosses user boundaries — Mark's profile shouldn't carry other contacts' dispositions). **Distinct from Mira item 14** (no names in the *Operator* profile). In v0.2 she has nowhere proper to put cross-user calibration, so it lands in the gatekeeper's profile. *Fix:* v0.3 — cross-person consolidation into `evryn_knowledge` is Reflection's job (ARCHITECTURE, Insight Routing Pipeline). v0.2 stopgap (Mira): keep per-user notes to per-user observations; don't write cross-contact maps into a user's profile.

### Open question — operator visibility into the live pipeline (for AC02 to crunch)
