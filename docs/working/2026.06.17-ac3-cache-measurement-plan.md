...
**Update 2026-06-23 — the SDK exposes NO cache breakpoints (issue #89); the lever to test is placement, not TTL (ADR-044 / sprint Steps 69–70).** AC3a's SDK research (claude-code-guide, 2026-06-23, against the Agent SDK TS reference + GitHub issues #89/#188/#311) confirmed `query()` exposes **no** `cache_control` / breakpoint control — breakpoint placement is SDK-internal and opaque. **Consequence:** we cannot *chunk* the cache via the API; the only lever is **placement** — what sits in the `systemPrompt` (cached prefix) vs. the first user message (uncached). That is exactly **ADR-044**: curated `story` in the `systemPrompt`, churning `pending_notes` in the user message. So this measurement now decides two things: **(1)** are we cold (Q1/Q2) → is cache work worth it at all; **(2)** would the ADR-044 restructure pay off (gated on (1)). **The measurement RUN is now sprint Step 69; the restructure is Step 70 (measurement-gated).** Critical timing: to measure the *current* construction, **leave `pending_notes` IN the `systemPrompt`** for the measurement window — moving them out (Step 70) destroys the ability to measure today's behavior. And lean Reflection (Step 10, shipped this wave) is the complementary lever — it keeps the per-user profile small + stable regardless of the cache outcome (Q4 measures its payoff).
