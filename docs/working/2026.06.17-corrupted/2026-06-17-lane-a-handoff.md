...
## Migration / DB — ✅ DEV-APPLIED + verified (2026-06-17, AC1); PROD apply rides the deploy

Both Lane A migrations are **applied to the DEV rehearsal DB (`Evryn Product — Dev`, us-west-2) and verified, 2026-06-17 by AC1** (Justin assigned the dev rehearsal to AC1 — "it's yours, do it"; AC0 had not done it). Applied via the full-path PG17 psql + `SUPABASE_DB_URL_DEV` per `backups/README.md`, each under its own `BEGIN/COMMIT` + `ON_ERROR_STOP=1`:
- **Step 13** (`backups/2026-06-17-emailmgr-items-updated-at-trigger.sql`): `DROP TRIGGER IF EXISTS` (NOTICE-skipped — confirms the idempotency guard) → `CREATE TRIGGER`. **Verified:** `trg_emailmgr_items_updated_at` present on `emailmgr_items` (reuses the prod-proven `public.touch_updated_at()`, same wiring as `trg_users_updated_at`).
- **Step 61** (`backups/2026-06-17-gatekeeper-address-resolution.sql`): table + 4 `COMMENT`s + index + `users.outbound_address` column + `GRANT service_role` + `REVOKE anon/authenticated`. **Verified:** `gatekeeper_inbound_addresses` (cols `address`/`gatekeeper_user_id`/`created_at`) + `users.outbound_address` (citext) both landed.
- No behavioral trigger-test was run (intentional): `touch_updated_at()` keys off the transaction timestamp, so a one-txn INSERT+UPDATE would false-negative; the trigger reuses the prod-proven convention, so existence + reuse is the verification. Real behavioral confirmation = stale-detection running against live `updated_at`.

- **🔴 PROD apply is NOT done — it rides the coordinated go-live deploy** (AC0 convergence + Justin's deploy-go). The prod recipe (pre/post `pg_dump` per `backups/README.md` + the file headers) still applies for prod; apply **both** Lane A migrations together at the deploy. The dev rehearsal de-risks it: both files are valid SQL, apply cleanly, and are idempotent (re-runnable guards), so the prod apply is mechanical.
- **PG17 client tools** are now installed on this machine; `SUPABASE_DB_URL_DEV` is in the worktree `.env` (Justin copied it in for the rehearsal).
...
- **Migration apply:** ✅ **DEV-APPLIED + verified 2026-06-17 (AC1)** — see §Migration/DB. PROD apply still rides the go-live deploy (both Lane A migrations together).
