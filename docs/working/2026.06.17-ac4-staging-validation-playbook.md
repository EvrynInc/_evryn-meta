...
> **⚠️ ROUND-2 UPDATE (2026-06-25):** the **send-allowlist is now BUILT + QC-GO** (`src/email/client.ts` `7705675`; the "Dependencies" note below saying it's unbuilt is stale), and staging's DB is now its **own dedicated 4th Supabase project** per [ADR-045](../decisions/045-staging-dedicated-database.md), NOT the dev DB. The finalized round-2 deltas — **validate by real behavior not `/health`**, the allowlist must include `review@evryn.ai`, and the clustered-model breadcrumb — live in the handoff `docs/working/2026.06.24-ac4a-ac0-lane-staging-r2-handoff.md` (§5). Kept here as the procedure skeleton; OC authors the runbooks from this + the handoff at standup.

> **Truncation check:** the last line of this file should read `FULL FILE LOADED`. If you don't see it, reload.

> **How to use this file:** The repeatable procedure for proving a release candidate works *as a whole system* on the staging runtime before it touches prod. Companion to the design doc `_evryn-meta/docs/working/2026.06.17-ac0-staging-launch-space.md` (that = *what staging is + why*; this = *how we validate through it*). It does **not** re-describe the Phase-6 scenarios — those live in `evryn-backend/tests/phase6-live-fire-v02.md`; this playbook says *how to run them against staging instead of prod, and what's staging-specific.* Read the design doc first. **Status: spec — gated on the staging runtime existing (and on the send-allowlist build, see Dependencies).**
