...
Thank you for the drafts — the structure is solid and the email forwarding pathway is well-covered. I can see the questionnaire came through clearly in how you approached the data handling and email processing sections.

I'm assuming these drafts are scoped to Phase 1 — getting us through Mark and the first few users who forward their inbound to us. I'm answering with that assumption, so I'm not flagging things that are obviously "full product" concerns. But there are some items where even for Phase 1, I think we need to get it right from the start, and a few things that have shifted since I sent the initial materials about a month ago.
...
**Privacy Notice — "customers" vs. "end users":** The customer/end user distinction doesn't quite fit our model. Everyone who interacts with Evryn is a "user." Mark is a user. The people who email Mark and get identified by Evryn are users. The difference is that Mark has a specific role — he forwards email, Evryn evaluates it, and everyone who comes through his inbound becomes a full Evryn user in their own right. I don't think we need a special label for that role in the Terms or Privacy Notice — we can just describe what those users do (e.g., "users who forward their inbound communications to Evryn for evaluation"). So I'd suggest: everyone is simply a "User," with the forwarding relationship described functionally where needed rather than given a distinct role label. Happy to discuss the best way to structure this.

**Privacy Notice — accounts:** Yes, accounts (user records) are created. Importantly: Evryn creates a user record for every person who emails Mark as soon as that email is forwarded — before the person has any interaction with Evryn or even knows Evryn exists. During Phase 1 (calibration), we're only notifying Mark — we don't contact any of those people. But we are storing their name, email address, message content, and Evryn's classification and reasoning about them. I think this needs to be disclosed clearly, especially since Phase 2 (outreach to those people) is only a few weeks later. The Evryn Gatekeeper Flow document I sent on March 6th describes this in detail — particularly the Phase I / Phase II structure and how user records are created from forwarded email.

**Privacy Notice — other PI categories:** The additional categories (conferences, job applications, business development) are fine to keep for coverage. On "Interactive Features" referencing social media pages — we don't currently have public sharing features, but we may feature user success stories on social media or our website in the future, always with the user's explicit consent. So the category is fine to keep, but I'd want to make sure it's clear that any public use of user-provided content requires explicit permission.
...
Users like Mark who forward their inbound to Evryn enter a specific operational relationship — forwarding email, data handling for forwarded content, expectations about what Evryn does with that data. One important principle (covered in the questionnaire and the Evryn Gatekeeper Flow document sent March 6th): Evryn's obligation is to each user individually. People who come through a user's inbound become full Evryn users in their own right — they are not the forwarding user's "leads," and no user has ownership or priority rights over the people in their inbound.
...
Section 3.6 says automated assessments "do not produce legal or similarly significant effects on individuals." I'm not sure this is accurate for our model. Evryn's classifications determine whether someone gets surfaced to a user who forwarded their email — which for a filmmaker trying to reach Mark, could meaningfully affect their professional opportunities. And once we're in Phase 2, Evryn's trust assessment determines what connections a user is offered, which is the entire value of the service.

I'd rather be transparent about this than minimize it. Can we revise to acknowledge that automated assessments meaningfully influence service delivery (which connections are offered), while noting that all classifications are currently subject to human review before action is taken?
...
The Evryn Gatekeeper Flow document (sent March 6th) describes this structure in detail — particularly the Phase I calibration period where emails are logged but no one is contacted, and the Phase II transition where outreach begins. I think the Privacy Notice needs to address this data collection — both what we collect and the legal basis for it. The Gatekeeper Flow should have the structural detail you need.
...
A few things have evolved in the month since I sent the initial materials that you wouldn't have known about:
...
8. Verify the forwarding relationship and "no ownership" principle are adequately covered
9. Add: crisis protocol / safety escalation disclosure
...
16. Implement affirmative "I agree" acceptance
17. Terminology: use "match" instead of "gold" in all public-facing language
