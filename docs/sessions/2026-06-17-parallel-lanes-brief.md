...
## Part 1b — Per-step solution shapes (the smaller steps + details; design intent, clean/junk flagged)

> The sprint has each step's *"what."* This is the *shape we converged on* (for the rewrite). Pattern: nearly all are **clean AC2/sprint designs with junk DC implementations** — the *designs* are reusable, the *code* is scrapped.
...
- **Step 26:** drop `Gold/Edge/Reply` from the operator subject (classification stays in the review body). **Discovery:** `findPendingByApprovalSubject` is dead code (no caller — approval is short-id-only), so the subject format is free to change. `[AC2, incl. the dead-code grep; DC#1-junk]`
- **Step 27:** move the approve/notes handle to directly under `--- END DRAFT ---`. `[AC2 + sprint; DC#1-junk]`
- **Step 28:** the collision help-string pointed at a non-existent "approve via full-UUID `supabase_upsert`" path → replace with a real remediation (clear one item out of `pending_approval` first; approval is short-id-only). `[AC2; DC#1-junk]`
- **Step 31 — KEY DISCOVERY:** the existing `tests/test-slack-parsing.ts` tested a re-implemented **OBSOLETE** parser (a *ghost*), so the real strict `approve <id4>` parser (the ADR-035 silent-diversion guard) had **zero real coverage**. Fix: export + test the *real* `parseSlackMessage`. **Carry this forward — a genuine latent risk regardless of this lane.** `[AC2 confirmed the ghost; sprint suspected zero coverage; DC#1-junk impl]`
- **Step 32:** Lane B owns the `executeApproval` retry/rollback failure-path test (inject its deps); AC1 owns the `client.ts` `sendEmail` transport-DI + its surfacing test. **Compose seam:** AC1's `sendEmail` gains a `transport` param → a clean Step 14 must match that signature. `[AC1 caught the split; AC2 design; DC#3-junk]`
- **Step 33:** content-based idempotency on `submitDraftForApproval` — same to/subject/body while `pending_approval` = true duplicate → early-return (no re-send/re-ping); a differing body = legitimate revision → proceeds. `[AC2 design; DC#3-junk]`
- **Step 37 — CLEAN, and NOT part of the junk-B1 chain:** `redact_user_from_message`, operator-gated (behind `threadScopeContext`, like `rescope_messages`); replaces a term with `[redacted user]` in `content_raw`+`message_body`; records *that* a redaction happened in metadata WITHOUT storing the term; case-insensitive + regex-escaped. Its `operator.md` repoint (raw `supabase_upsert` → this tool) is the tool's natural beat — **distinct from the junk status repoints.** `[AC2 design; DC#3-junk]`
- **Deferred code nits (JUNK — QC#4/#6-sourced; re-derive only if they survive a clean rebuild):** (a) the redact tool logs `redact_text` to infra logs (a containment-tool leak); (b) `redactInMessages` stamps `redacted:true` even on a zero-occurrence message; (c) the Step-32 success-path delivered-flip uses the real `updateEmailmgrItem`, not the injected one.
- **Convergence merge seam (from the original brief's overlap map — a note for AC0, not a pitch):** `classify.ts` is shared — Lane B's MCP-tools region vs Lane C's `loadCommonPrefix`/`runEvrynQuery`; the `allowedTools` array is the likely collision point.
- **Slack pings (no artifact to fix):** my `#team-alerts` taps to Justin asserted "QC-GO" / "B1 fully closed" — now **invalid** (the QC verdicts were junk). Ephemeral DMs to Justin, not a lingering channel record; noted for completeness.
...
3. **→ Mira (via AC0's batch): the identity beats — SPLIT clean vs junk** (correction to my first pass, which lumped these together):
   - **JUNK (B1-derived):** the *status* repoints (ignored/bad_actor/matched → `set_item_status`), the bad_actor user-record stamp + note, and the matched terminalization — all downstream of the suspect B1/B1-a findings. **Don't voice/ship until a clean DC/QC re-confirms the need.** `[SUBAGENT: QC#2/#4 + Justin]`
   - **CLEAN (Step-37 design, NOT B1-derived):** the operator-redaction repoint (raw `supabase_upsert` → `redact_user_from_message`) — the redact tool's natural identity beat. Carry forward once the clean redact tool is rebuilt. `[AC2 design]`
...
> **Why this exists:** the subagent-load protocol was **CONFIRMED misfiring** this wave (identity/context files weren't synced — QC's for sure, DC's too), so **ALL Lane A subagent work is junk** and the worktree `evryn-backend-ingest` is being shelved. Lane A spun **7 subagents — 4 "DC" builds** (`0430a6f`, `30db607`, `3f8d3d4`, `d2d98af`) **+ 3 "QC" reviews** (all returned a now-meaningless GO). This is the **complete, canonical Lane A capture** for AC0's re-brief (it absorbs the temporary standalone fallback doc, now deleted — there was a brief detour where I kept the full set in a standalone while this shared brief was being concurrently appended; it's all folded back in here).
>
...
2. **Step 17 empty-body bounce = DIRECT send bypassing the approval gate — allowed ONLY because deterministic/fixed-template/no-LLM/no-PII (pre-authorized content), respects `SEND_ENABLED`, audited to `messages`.** Bounce *copy* = placeholder → **Mira owns wording; must stay deterministic** (the whole basis for the bypass). *(Justin, ballot A. Gate-tension was AC1-clean from ARCHITECTURE's Outbound-Approval-Gate. The "`sendEmail` isn't an MCP tool" reassurance was ⚠️ SUBAGENT — re-confirm clean.)*
   - **Known edges the clean build must handle deliberately:** (a) empty-body emails create **no `emailmgr_items` row → no durable dedup** (only in-memory, lost on restart) → a restart can **re-bounce** an empty email still in the inbox; (b) repeated empty sends → repeated bounces (**no dedup-guard** built — accepted low-risk at v0.2); (c) the bounce-vs-skip decision must come **after a cheap no-LLM sender lookup** (known→bounce; unknown→silent skip + dev-alert), and lookup/send failures must degrade to **transient retry**, not silent drop or blind bounce. *(Edges (a)/(b) ⚠️ SUBAGENT-surfaced + my review; the after-lookup structure + transient-degrade is AC1-clean/Justin's design.)*
3. **Step 21 cursor resilience = time-bounded catch-up, NO new table.** Cold-start/`historyId`-expiry → fetch "everything since the last processed email" (from existing data — newest `emailmgr_items.created_at`, minus a small margin), via a **lazy provider** (DB read only on a fallback, not every poll); larger page; dedup absorbs the overlap; replaces the "last 10 inbox" fallback that dropped mail on restarts. Keep the fetch layer DB-free (it receives the provider closure). *(AC1-clean; no-new-table was my call. Justin approved, ballot B.)*
   - **Known limitation to decide deliberately:** the fallback **page size bounds how long an outage it covers** (junk build used `maxResults: 100` ≈ ~12h at Mark's ~200/day, no pagination) — pick a sensible size or paginate on `nextPageToken`. *(⚠️ SUBAGENT-surfaced but a real property of the design.)*
4. **Step 32 = SPLIT** — Lane A owns only the `client.ts` `sendEmail` injectable-transport seam (default = real Gmail send; a throwing transport must propagate so a caller's retry has a real failure surface) + its surfacing test; the `executeApproval` retry/rollback test → **Lane B**. *(AC1 caught the seam; AC0 formalized; Justin ballot D.)*
5. **Step 13 must be a DB TRIGGER reusing `public.touch_updated_at()`** (already on `users` via `trg_users_updated_at`), NOT `moddatetime`; trigger is required because Evryn's raw status writes bypass `updateEmailmgrItem`; make it idempotent (`DROP TRIGGER IF EXISTS`). *(Trigger-not-app-code is AC1-clean. "Reuse `touch_updated_at` + `DROP TRIGGER IF EXISTS`" was ⚠️ SUBAGENT (trip-1 QC) but I independently verified it against the real schema dump → sound.)*
6. **Step 61 (gatekeeper-address resolution) — full design exists, is the cleanest surviving artifact.** Inbound lanes (many→one via a `gatekeeper_inbound_addresses` table, `address citext PK` → `gatekeeper_user_id`) + settable `users.outbound_address` (gold destination, decoupled from inbound; falls back to `users.email`) + **escalate unregistered-lane forwards** (don't triage blind — the bug Justin reacted to). Email-only v0.2 floor; multi-channel = v0.3. *(Design = AC1-clean (pre-Step-61-subagent) + Justin's all-(a) ballot + AC2/AC0's outbound model. Doc: `docs/working/2026-06-17-step61-gatekeeper-address-resolution.md`. Build = ⚠️ SUBAGENT.)*
   - **Load-bearing design principles inside Step 61 (do NOT lose — they prevent silent failures):** **(i) the lane resolver must THROW on a DB error, NOT swallow-to-null** — a swallowed error looks like a clean "no lane" miss, which routes a forward to escalate-or-blank-lead, so a transient blip would **misroute a real gatekeeper's forward**; throw → caller treats it transient → hold cursor + retry. *(AC1-clean — specified in my DC brief.)* **(ii)** resolution is a pure branch: lane-hit → canonical gatekeeper (no blank lead); forward + no-lane → **escalate** (short-circuit, no triage); not-forward + no-lane → existing find-or-create-as-lead; dedup stays FIRST. *(AC1-clean.)* **(iii) escalation mechanics:** a tracked `emailmgr_items` row stamped `escalated`, **anchored to the Operator system actor** for the NOT-NULL `user_id` (real sender in `metadata.unresolved_from`), `external_id`-stamped for re-fetch dedup, + an operator ping; the 4h stale-`escalated` checker is the backstop. *(Operator-anchor was a ⚠️ SUBAGENT impl choice within my "least-bad FK, DC decides" note — re-confirm.)*
7. **Step 61 outbound/threading model:** gold → the gatekeeper's *designated* destination; threads into the original only if that destination is the same account holding it (AC2 headers), else standalone w/ original subject quoted. No cross-account threading. Lane A SETs destination + STASHes original subject (on the item); **AC2 Step 14 USEs them** (reads `outbound_address` off the gatekeeper record, `original_subject` off the item). *(AC2 via Justin + AC0.)*
8. **Minor builds (AC1-clean designs; implementations junk):** Step 16 internal-address sender-skip (skip `review@evryn.ai`-type by sender, like the self-skip; kills the phantom-user cascade); Step 19 external-id dedup fallback (`messageId||gmailId` via one shared helper used by the dedup-check AND both item-creates); Step 20 rate-limit the transient alert (generalize the checker-cooldown) + **remove the double-`notifyDev`** (inner one in `process.ts`'s `findOrCreateUser` catch); Step 22 hour-gate the `[cron:proactive] woke` heartbeat; Step 29 comment accuracy (re-processing is safe by *dedup*, not by post-write throws being impossible); Step 40 drop the stale "test recipient override" clause from `client.ts`'s `sendEmail` comment (`TEST_RECIPIENT` is dead in `src/`; `NODE_ENV` cosmetic — any cleanup is ops, not code).
9. **Orchestration / process:** ran as two batches (mechanical-now vs design-pending-on-ballot); AC0 added "post-and-proceed"; the migration dev-rehearsal is the Lane-A AC's job ("AC applies"), Justin confirmed. My **full-cascade load preceded any subagent** (why my runtime reads are clean); I **read every diff** (real AC reviews) — but an AC review is **not** a substitute for independent QC, and the QC here was suspect. *(AC1+AC0+Justin.)*

## Part 2 — Pitched to OTHER lanes (AC0: roll into THOSE briefs)

- **→ Lane B:** (a) the Step 32 `executeApproval` retry/rollback test; (b) `sendEmail` gained a `transport` param — Step 14 must compose with that signature; (c) **AC2's Step 18 `set_item_status`** — Lane A's `checkFollowUps` prompt must reference it (not `supabase_upsert`), a **convergence dependency** (prompt + tool same bundle); a `process.ts` comment was also swapped to match. *(swap commit `d2d98af` is junk; clean re-run re-adds once Lane B's clean Step 18 exists.)*
- **→ AC0/sprint:** fix the Step 34 framing (Part 1 #1); **Step 58 clustering** touches `poll.ts` — seam = the synchronous per-email triage trigger (`handleNewEmail`→`processEmail`→`processForward/Direct`→`runEvrynQuery`); a daily-cluster cron joins the existing crons. **Step 57 (runtime-does-bookkeeping, the Phase-2b cost keystone) will ALSO rework `process.ts`** (a Lane A file) — a future cross-lane seam the re-spin's Lane A should coordinate with the cost thrust. *(AC0 Steps 57/58 + AC1-clean seam analysis.)*
- **→ ARCH/BUILD + v0.3 (auth-gated — AC0/Justin apply):** Step 61 implies ARCHITECTURE edits (Identity resolution / Gatekeeper users / Data Model / Pipeline sender-resolution) + BUILD (gatekeeper model / workflow / onboarding) — **drafted verbatim in the Step 61 design doc's "Proposed doc edits," NOT applied; re-review with a clean AC.** That doc also carries a **v0.3-scaling note Justin pre-authorized**: the escalate-unregistered-forward floor *doesn't scale* (manual operator adjudication is untenable at multi-gatekeeper / v0.3 self-signup / cast-off volume) → v0.3 needs a real trust-based design (sibling to "Gaming-Resubmit Detection at Scale"). Drafted in the design doc, not yet in ARCH/BUILD/SPRINT-v0.3. *(Justin pre-authorized; AC1 drafted.)*
...
4. **evryn-meta commits** on `main` (`393228d`/`7952ed3`/`44ab67e` + the standalone-capture commits `6c4319b`/`0146ea4`, now superseded by this inline addendum, + the commit folding it in/deleting the standalone) — they added the working docs above. Survive (different repo from the worktree).
5. **Slack pings** announced "Lane A QC-GO" / "dev rehearsal done" — **now false.** Not files; a one-line `#dev-alerts` retraction would set the record straight (specialty team / operator's call).
6. **Build-detail seams for the record:** the Step-21 design needs a "newest item timestamp" DB read; the junk build put an additive helper (`getNewestEmailmgrItemCreatedAt`) in `src/db/items.ts` (**Lane B's region** — additive, not `updateEmailmgrItem`) — a minor cross-lane placement to resolve. `EVRYN_USER_ID`/`OPERATOR_USER_ID` were added as **local constants** in `poll.ts`/`process.ts` (existing pattern; centralizing is the separate deferred Step 42).
7. **The suspect QC also raised minor nits** (the bounce echoes the sender's own subject; the `escalated` item has no `lifecycle[0]` entry; the escalation `notifySlack` writes no `messages`-audit row) — **minor; drop, let a clean QC re-derive.**
8. **NO `CLAUDE.md`/identity/Linear changes** (Lane A was pure-runtime). **Housekeeping:** the worktree `.env` (live DB creds) — remove it if you *shelve* rather than delete the worktree.
