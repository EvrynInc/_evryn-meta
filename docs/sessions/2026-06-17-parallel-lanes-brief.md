...
1. **DROP Step 11a (switch to 1h cache TTL) — it's MOOT.** *(Justin — your decision to cluster the day's reads into one ~10am batch (Step 58) keeps the cache warm by volume, replacing the TTL lever. A `claude-code-guide` subagent also claimed the Agent SDK exposes no TTL knob — **(SUBAGENT — suspect), re-verify** — but the moot conclusion stands on your clustering regardless of that claim. The research also surfaced one *possible* TTL lever — a `ENABLE_PROMPT_CACHING_1H` env var Claude Code honors that MIGHT thread to the SDK subprocess — captured in the measurement plan as an **untested contingency; (SUBAGENT-sourced — suspect).**)*
...
11. **Verified (Justin asked) — passes do NOT bloat the gatekeeper's profile.** On a `pass`, `record_pass` writes its deterministic stamp to the **contact's** record only, **never the gatekeeper's** (a hard invariant in the code); plus Mira's note-discipline = zero prose pass-notes. So Mark's profile growth — the tax Reflection targets — is legit **gold/edge + onboarding** accumulation, not passes. *(AC — confirmed by reading `record_pass`. Clean; a premise worth stating in the cost re-brief.)*
...
- **Starting defaults (AC — derived from the cost model; tunable, not load-bearing):** consolidate when a user's pending-notes exceed ~24,000 chars (~6k tok); soft story target ~10,000 chars (~2.5k tok — the cost-analysis consolidated-profile target); alarm (don't loop/truncate) if the new story exceeds ~4× the target.
...
- **→ Lane A:** the Reflection trigger is a poll-loop checker = **one line added to `poll.ts`** (Lane A's file). Same file Lane A flagged for Step 58 clustering — the re-build will touch it again; AC0 sequence the merge.
- **→ Step 58 (clustering):** Reflection is size-triggered, so it composes cleanly with daily clustering (fires naturally after a day's batch adds notes) — no coupling needed. *(AC.)*
