...
- **NEW → SPRINT Step 66 — DESIGN CALL FIRST (surface your plan in the 3-part form before building):** proactive outreach silently excludes the ENTIRE discovered-user population. `checkProactiveOutreach` (`src/email/poll.ts`) enrolls via `findUsersByStatus(['active','gatekeeper'])` — but `'gatekeeper'` is NOT a valid `users.status` value (dead token), and `'lead'` (the DEFAULT status of every discovered/forwarded user) is not in the list — so a discovered user NEVER receives a proactive check-in. This contradicts ARCHITECTURE ("discovered users become full Evryn users"; proactive care is "core Evryn"). The fix needs a decision — when/how does a `lead` become `active`, or should the enrollment filter change? — so plan it with Justin before building.
- **NEW → SPRINT Step 20 (audit-confirmed + sharpened):** the `handleNewEmail` transient-failure alert (`src/email/poll.ts`) is NOT rate-limited — the per-checker cooldown only wraps the cron checkers, so a sustained Supabase/Gmail outage floods `#dev-alerts` every poll cycle (~30s). Generalize the existing checker-cooldown mechanism to cover this alert too.
...
- **NEW → SPRINT Step 63 — generalize the fabricated-UUID guard:** the MCP tools `append_pending_note`, `append_cross_user_note`, `set_thread_scope`, `rescope_messages`, and `notify_slack(about_user_id)` (all in `src/triage/classify.ts`) accept an Evryn-supplied `user_id` as a plain `z.string()` with NO UUID-format or scope/relation validation. The `record_pass` "B-1" hardening — which stopped trusting an Evryn-supplied id precisely because she can fabricate a UUID — was never generalized to these, so a wrong/fabricated UUID lands a write on the WRONG user's record. Mirror the record_pass B-1 fix on each.
- **NEW → SPRINT Step 64 — audit-write recipient mislabel:** in `executeApproval` (`src/approval/flow.ts`), the best-effort audit `createMessage` sets `recipient_id = (await findUserByEmail(draft.draft_to))?.id ?? EVRYN_USER_ID` — so when the recipient lookup returns null, the outbound is silently recorded as **Evryn→Evryn** (a corrupt conversation-history row; worse at v0.3 where recipient ≠ gatekeeper). Fix: use the authoritative `item.user_id` for the recipient, or `notifyDev` on the null branch instead of self-addressing.
- **NEW → SPRINT Step 65 — collision-reply threading + dead remediation:** the collision / no-match / lookup-failed branches in `handleApprovalMessage` and `handleNotesMessage` (`src/notify/slack.ts`) call `say(...)` WITHOUT a `thread_ts`, so the reply posts as a top-level channel message (leaking another item's subject + full UUID to the channel, and breaking thread placement) — AND they tell the operator to "approve by the full UUID via `supabase_upsert`," a path that does NOT exist (approval is short-id-only). Existing Step 28 covers only the *stale-checker* copy of that wrong string; make your fix cover BOTH this live collision branch and Step 28's.
- **NEW → SPRINT Step 67a — revision-prompt history double-attach:** `handleRevisionNotes` (`src/approval/flow.ts`) builds a `formatConversationHistory` block and appends it to the prompt, while `core.md` tells Evryn the runtime owns history and she must NOT attach her own → a revised draft can double-attach history. Resolve whether the runtime should stop handing her history on the revision path, or the identity files should account for it — **coordinate the identity side with Mira.**
- *(Already on your list above: item 10 — the triage-reasoning-on-downgrade capture.)*
...
**AUDIT-DERIVED ADDITION (foundation audit, 2026-06-22):** minor, for the cost work — `recordLlmUsage` (`src/triage/classify.ts`) is deliberately NOT called on a query that throws mid-stream (Anthropic 5xx, billing error, max-turns abort), so errored queries silently under-count spend with no error-path accounting. Not blocking; account for it when you build the runtime-bookkeeping (Steps 57/4) so the cost picture isn't quietly low on error-heavy days. Findings doc: `_evryn-meta/docs/working/2026-06-22-foundation-audit-findings.md`.
...
- **NEW (sharpens your watchdog design):** the `/health` endpoint (`src/index.ts`) ALWAYS returns `200 {status:"ok"}` regardless of actual liveness — Supabase/Slack/polling can all be dead while `/health` still reports healthy. So the "Railway healthcheck-failure → external watchdog" path named in Step 4 is NOT viable as written (the health check can never fail because it never reflects real health). Either make `/health` reflect real liveness so Railway's healthcheck can catch a wedged process, or pick a different external-watchdog signal.
- **NEW → SPRINT Step 67b (cosmetic):** `src/notify/emergency.ts`'s JSDoc references a `bypassQuietHours: true` flag on `notifySlack` that no longer exists (retired in the quiet-hours redesign). Trim the stale comment so it doesn't mislead a future maintainer reading the emergency-channel design.
