...
**Upcoming (current 4-lane wave — each deploys on Justin's go, then gets its number):** AC4 security **Bundle 2** (constrained `supabase_read`, `ac4/sec-bundle2` @ `547bc0b`, QC-GO, parked) → next `v0.2.N` · AC2 cost lane (`dc/cost-haiku-prescreen`) → after · (AC3 identity/ops rides where its pieces land). Numbers are assigned **at deploy**, never reserved ahead. *(AC4 security Bundle 1 shipped as v0.2.4, above.)*
