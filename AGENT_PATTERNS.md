...
Learnings from building AI agents that will help when building Evryn (the product) and vice versa.

This file is not loaded by *anyone* during builds - this is just a message board so that others can learn, cross-repo. But this never replaces capturing this info into places that *will* be reliably retrieved when it's time to build.
